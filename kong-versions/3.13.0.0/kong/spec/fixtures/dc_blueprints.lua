...
local function wrap_db(db, expand_foreigns, preserve_nulls)
  local dc_as_db = {}
  ...
  for name, _ in pairs(db.daos) do
    dc_as_db[name] = {
      insert = function(_, tbl)
        ...
        if not preserve_nulls then
          tbl = remove_nulls(tbl)
        end
        table.insert(config[name], tbl)
...
function dc_blueprints.new(db, expand_foreigns, preserve_nulls)
  local dc_as_db = wrap_db(db, expand_foreigns, preserve_nulls)
