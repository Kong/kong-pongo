...
describe('Vaults: Secret referencing in AWS-Lambda plugin', function () {
  ...
  before(async function () {
    ...
    await createHcvVaultWithApproleInKong(
      hcvMount,
      hcvPrefixWithApproleAndResponseWrapping,
      hcvApproleID,
      hcvWrappedSecretID,
      true,
    );
  ...
  it('should create hcv vault entity and secrets', async function () {
    ...
    expect(resp.data.aws_secret_key, 'Should see aws secret ket').to.equal(awsSecretKey);
    expect(resp.data.aws_access_key, 'Should see aws access key').to.equal(awsAccessKey);
  });
  ...
  it('should reference with aws access and secret keys hcv vault entity', async function () {
    ...
    expect(patchResp.status, 'Status should be 200').to.equal(200);
    expect(patchResp.data.config.aws_secret, 'Should replace aws_secret with secret key reference').to.equal(
      `{vault://${hcvPrefix}/${hcvSecretPath}/aws_secret_key}`,
    );
  ...
  it('should reference with aws access and secret keys hcv vault secrets', async function () {
    ...
    expect(patchResp.status, 'Status should be 200').to.equal(200);
    expect(patchResp.data.config.aws_secret, 'Should replace aws_secret with hcv secret key reference').to.equal(
      `{vault://hcv/${hcvSecretPath}/aws_secret_key}`,
    );
  ...
  it('should reference with aws_access_key hcv and secret_key hcv vault entity secret with approle', async function () {
    ...
    expect(patchResp.status, 'Status should be 200').to.equal(200);
    expect(patchResp.data.config.aws_secret, 'Should replace aws_secret with hcv secret key reference').to.equal(
      `{vault://${hcvPrefixWithApprole}/${hcvSecretPath}/aws_secret_key}`,
    );
  ...
  it('should reference with aws_access_key hcv and secret_key hcv vault entity secret with approle and response wrapping', async function () {
    ...
    expect(patchResp.status, 'Status should be 200').to.equal(200);
    expect(patchResp.data.config.aws_secret, 'Should replace aws_secret with hcv secret key reference').to.equal(
      `{vault://${hcvPrefixWithApproleAndResponseWrapping}/${hcvSecretPath}/aws_secret_key}`,
    );
  ...
  it('should reference with aws_access_key env and secret_key aws secrets', async function () {
    ...
    expect(patchResp.status, 'Status should be 200').to.equal(200);
    expect(patchResp.data.config.aws_secret, 'Should replace aws_secret with secret key reference').to.equal(
      '{vault://aws/gateway-secret-test/aws_secret_key}',
    );
