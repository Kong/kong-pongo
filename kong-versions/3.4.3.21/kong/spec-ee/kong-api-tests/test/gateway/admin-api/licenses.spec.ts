...
describe('@gke: Gateway /licenses API tests', function () {
  ...
  const assertBasicRespDetails = (resp: AxiosResponse) => {
    expect(resp.data.created_at, 'Should have created_at entry').to.be.a('number');
    expect(resp.data.id, 'Should have id').to.be.string;
    expect(resp.data.updated_at, 'Should have updated_at entry').to.be.a('number');
  ...
  it('should GET all licenses and see the existing license', async function () {
    ...
    expect(resp.data.data, 'Should see 1 license in licenses').to.have.lengthOf(1);
  ...
  it('should not PATCH create a valid license', async function () {
    const resp = await postNegative(`${url}/${sampleId}`, { payload: validLicense }, 'PATCH');
    logResponse(resp);
  ...
  if (isCIrun) {
    it('should not enable key-auth-enc ee plugin without license', async function () {
      ...
      expect(resp.status, 'Status should be 400').to.equal(400);
      expect(resp.data.message, 'Should see correct error message for plugin creation').to.include(
        `'key-auth-enc' is an enterprise only plugin`,
      );
    });
  }
  ...
  it('should not POST a duplicate valid license', async function () {
    ...
    expect(resp.data.message, 'Should see correct error message').to.include('UNIQUE violation detected');
  });

  it('should not PUT a duplicate valid license with different id', async function () {
    const resp = await postNegative(`${url}/${sampleId}`, { payload: validLicense }, 'PUT');
    logResponse(resp);
  ...
  it('should not PATCH an existing license with invalid poayload', async function () {
    const resp = await postNegative(`${url}/${licenseId}`, { payload: inValidLicense }, 'PATCH');
    logResponse(resp);
  ...
  it('should PATCH an existing license with valid poayload', async function () {
    const resp = await postNegative(`${url}/${licenseId}`, { payload: validLicense }, 'PATCH');
    logResponse(resp);
  ...
  it('should PUT a duplicate valid license with same primary key id', async function () {
    const resp = await postNegative(`${url}/${licenseId}`, { payload: validLicense }, 'PUT');
    logResponse(resp);
  ...
  it('should GET the license report', async function () {
    ...
    expect(resp.status, 'Status should be 200').to.equal(200);
    expect(resp.data.license.license_key, 'Should see correct license key').to.eq(licenseKey);
    expect(resp.data.plugins_count, 'Should see plugins_count in response').to.exist;
    expect(resp.data.deployment_info, 'Should see deployment_info in response').to.exist;
    expect(resp.data.timestamp, 'Should see timestamp in response').to.exist;
    expect(resp.data.checksum, 'Should see checksum in response').to.exist;
    expect(resp.data.system_info, 'Should see system_info in response').to.exist;
    expect(resp.data.counters.buckets[0].request_count, 'Should see request_count').to.be.greaterThanOrEqual(0);
    expect(resp.data.counters.total_requests, 'Should see total_requests').to.be.greaterThanOrEqual(0);

    expect(resp.data.workspaces_count, 'Should see workspaces_count in response').to.eq(1);
    expect(resp.data.db_version, 'Should see db_version in response').to.include('postgres');
  });
