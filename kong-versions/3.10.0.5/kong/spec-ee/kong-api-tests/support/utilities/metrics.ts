...
import { expect, getBasePath, Environment, isGateway, getNegative, vars } from '@support';
...
/**
 * Queries the target metrics data from appdynamics
 ...
 */
export const queryAppdynamicsMetrics = async (serviceName, appName) => {
  const resp = await axios({
...
/**
 * Waits for service to appear in appdynamics
 * @param {string} serviceName - name of the service to wait for
 * @param {string} appName - name of the app to wait for
 * @returns {object} - response from AppDynamics
 */
export const waitForAppDService = async (serviceName, appName) => {
    const resp = await queryAppdynamicsMetrics(serviceName, appName)

    expect(resp.data.length, 'Should see appdynamics data results for search').to.be.gt(0)
    expect(resp.data[0].metricName, 'Should not have empty metric name').to.not.be.empty
    return resp
}