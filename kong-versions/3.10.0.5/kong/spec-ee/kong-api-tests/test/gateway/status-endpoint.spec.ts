...
describe('@oss: /status Endpoint tests', function () {
  ...
  it('should return 503 for CP status when connection to database is lost', async function () {
    ...
    runDockerContainerCommand(databaseContainerName, 'container wait');
  ...
  it('should return 200 OK for CP status when connection to database is restored', async function () {
    ...
    await eventually(async () => {
      const containerStatus = JSON.parse(runDockerContainerCommand(databaseContainerName, "inspect"))
    ...
    await waitForTargetStatus(200, 20000);
  });
});
