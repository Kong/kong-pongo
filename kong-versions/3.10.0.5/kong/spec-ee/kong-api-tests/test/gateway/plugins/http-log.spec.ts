...
describe('@oss: Gateway Plugins: http-log', function () {
  ...
  before(async function () {
    ...
    if (!checkForArm64()) {
      const isSplunkRunning = await isContainerRunningByImage('splunk');
      if(!isSplunkRunning){
        runDockerComposeCommand('--profile manual up -d splunk'); //Start the Splunk server
      }
  ...
  after(async function () {
    ...
    if (!checkForArm64()) {
      await deleteSplunkServerHttpLogs(); //Note: Run the step only on non-ARM architecture, as the Splunk server is only supported on this architecture.
      runDockerComposeCommand('rm -sf splunk');//Stop the Splunk server
    }
  });
});
