...
describe('Gateway Plugins: Datadog', function () {
  ...
  const v1MetricsApi = new v1.MetricsApi(dataDogConfiguration);
  ...
  before(async function () {
    ...
    const isDatadogRunning = await isContainerRunningByImage(datadogAgentContainerName);
    if(!isDatadogRunning) {
      const command = `${isPackageTest ? '-f docker-compose-package.yml ' : ''}--profile manual up -d ${datadogAgentContainerName}`; // Start datadog container
  ...
  it('should be able to retrieve custom metric from Datadog', async function () {
    /*
    * See datadog api reference page: https://docs.datadoghq.com/api/latest/metrics/#search-metrics
    *
    * A request is made with datadog client to search for the metric created in kong gateway, 
    * a query param is passed to the client: "q: `${datadogPrefixName}.request.count`"
    * 
    * Metric should be unique since is using a unique prefix name.
    */ 

    const params: v1.MetricsApiListMetricsRequest = {
      q: `${datadogPrefixName}.request.count`,
    };      
    
    await eventually(async () => {
      const metricListResults = await v1MetricsApi.listMetrics(params);
      expect(metricListResults.results?.metrics?.length, 'kong metric is listed in datadog server').greaterThan(0);
    });
  });
