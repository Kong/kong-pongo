...
describe('Gateway Plugins: request-callout', function () {
  ...
  before(async function () {
    ...
    const isSquidAndCaddyRunning = await isContainerRunningByImage('squid') && await isContainerRunningByImage('Caddy');
    if(!isSquidAndCaddyRunning) {
    runDockerComposeCommand('--profile manual up -d squid Caddy'); // Start squid and caddy containers
    }
  ...
  after(async function () {
    ...
    runDockerComposeCommand('rm -sf squid Caddy');// Stop squid and caddy containers
