 ...
 * @returns
 */
const sendSolaceMsgViaKong = async (url: string, msg: string, headers: any = {}) => {
...
describe('Gateway Plugins: Solace Upstream Plugin', () => {
  ...
  before(async function () {
    if (checkForArm64()) {
      console.warn('Skipping Solace tests on arm64 because the Solace Docker version does not support.');
      this.skip();
    }
    const isSolaceRunning = await isContainerRunningByImage('solace/solace-pubsub-standard');
    if (!isSolaceRunning) {
      runDockerComposeCommand('--profile manual up -d solace'); //Start the Solace server
    }
  ...
  it('should send a message to Solace', async () => {
    ...
    const resp = await sendSolaceMsgViaKong(`${proxyUrl}${solacePath}`, 'Hello Solace!');
  ...
  it('should successfully send a message to Solace queue without body forwarding with solace-upstream plugin', async () => {
    ...
    const resp = await sendSolaceMsgViaKong(`${proxyUrl}${solacePath}`, 'Hello Solace without body forwarding!');
    expect(resp.status, 'Status should be 200').to.equal(200);
  ...
  it('should send a message to Solace with all parameters forwarded', async () => {
    ...
    const resp = await sendSolaceMsgViaKong(url, message, header);
    expect(resp.status, 'Status should be 200').to.equal(200);
    expect(resp.data?.message ?? resp.data, 'Response should include message sent confirmation').to.include(
      'message sent as a non-guaranteed direct delivery',
    );
  ...
  it('should send a message to Solace with dead message queue enabled', async () => {
    const resp = await sendSolaceMsgViaKong(`${proxyUrl}${solacePath}`, 'Hello Solace with DMQ!');
    expect(resp.status, 'Status should be 200').to.equal(200);
    expect(resp.data?.message ?? resp.data, 'Response should include message sent confirmation').to.include(
      'message sent as a non-guaranteed direct delivery',
    );
  ...
  it('should send a message to Solace with guaranteed delivery mode', async () => {
    const resp = await sendSolaceMsgViaKong(`${proxyUrl}${solacePath}`, 'Hello Solace with guaranteed delivery!');
    expect(resp.status, 'Status should be 200').to.equal(200);
    expect(resp.data?.message ?? resp.data, 'Response should include message sent confirmation').to.include(
      'message sent',
    );
  ...
  it('should send a message to multiple destinations using solace-upstream plugin', async () => {
    ...
    const resp = await sendSolaceMsgViaKong(`${proxyUrl}${solacePath}`, 'Hello Solace with multiple destinations!');
    expect(resp.status, 'Status should be 200').to.equal(200);
    expect(resp.data?.message ?? resp.data, 'Response should include message sent confirmation').to.include(
      'message sent',
    );
  ...
  it('should send a message to the remaining destination using solace-upstream plugin', async () => {
    backupMessage = '';
    const resp = await sendSolaceMsgViaKong(`${proxyUrl}${solacePath}`, 'Hello Solace with one destination!');
    expect(resp.status, 'Status should be 200').to.equal(200);
    expect(resp.data?.message ?? resp.data, 'Response should include message sent confirmation').to.include(
      'message sent',
    );
  ...
  it('should not be able to send a message to Solace with wrong VPN name', async () => {
    await eventually(async () => {
      const resp = await sendSolaceMsgViaKong(`${proxyUrl}${solacePath}`, 'Hello Solace with wrong VPN name!');
  ...
  it('should deliver a direct message to topic successfully', async () => {
    ...
    await eventually(async () => {
      const resp = await sendSolaceMsgViaKong(`${proxyUrl}${solacePath}`, 'Hello topic!');
      logResponse(resp);
      expect(resp.status, 'Status should be 200').to.equal(200);
    });
  ...
  after(async () => {
    ...
    runDockerComposeCommand('rm -sf solace');
