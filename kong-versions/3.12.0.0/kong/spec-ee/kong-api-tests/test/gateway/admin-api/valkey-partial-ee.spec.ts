...
describe('@weekly: Gateway Valkey-Partial Tests', function () {
  const valkeyImage = 'valkey/valkey';
  ...
  before(async function () {
    ...
    const isValkeyRunning = await isContainerRunningByImage(valkeyImage);
    if(!isValkeyRunning) {
      // Start valkey container
      const command = `--profile manual up -d ${valkeyContainerName}`;
      runDockerComposeCommand(command);

      await eventually(async () => {
        const containerStatus = runCommandInDockerContainer(valkeyContainerName, `valkey-cli --user ${valkeyUser} --pass ${valkeyPassword} PING`);
  ...
  after(async function () {
    ...
    runDockerComposeCommand('rm -sf valkey'); 
