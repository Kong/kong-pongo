...
describe('Vitals with InfluxDB Tests', function () {
  ...
  before(async function () {
    ...
    const isInfluxDbRunning = await isContainerRunningByImage('influxdb');
    if(!isInfluxDbRunning){
      runDockerComposeCommand('--profile manual up -d influxdb'); // Start the influxdb service
  ...
  after(async function () {
    ...
    runDockerComposeCommand('rm -sf influxdb'); // Stop the influxdb service
  });
});
