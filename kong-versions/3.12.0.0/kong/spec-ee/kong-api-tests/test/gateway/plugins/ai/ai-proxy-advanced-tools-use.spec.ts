...
describe("@ai: Gateway Plugins: AI Proxy Advanced Tools Use", function () {
  ...
  for (const test of tests) {
      ...
      const jsonBody = {
        "messages": [
          ...
          },
          {
            ...
            "content": SETUP_MESSAGES.TOOL_CALL_01_CONSUMER_METADATA
          }
        ],
        "tools": TOOLS_AVAILABLE,
        "stream": isStream
      ...
      const jsonBody = {
        "messages": [
          ...
          },
          {
            ...
            "content": SETUP_MESSAGES.TOOL_CALL_02_CONSUMER_USAGE_STATS
          },
          {
            "role": "tool",
            "tool_call_id": toolCallIds.toolCall03,
            "content": SETUP_MESSAGES.TOOL_CALL_03_NEW_YORK_TEMP
          },
          {
            "role": "tool",
            "tool_call_id": toolCallIds.toolCall04,
            "content": SETUP_MESSAGES.TOOL_CALL_04_OSLO_TEMP
          },
          {
            "role": "tool",
            "tool_call_id": toolCallIds.toolCall05,
            "content": SETUP_MESSAGES.TOOL_CALL_05_MADRID_TEMP
          }
        ],
        "tools": TOOLS_AVAILABLE,
        "stream": isStream
      ...
      const jsonBody = {
        "messages": [
          ...
          },
          {
            ...
            "content": SETUP_MESSAGES.TOOL_CALL_06_SECRET_CIPHER_JACKT
          },
          {
            "role": "tool",
            "tool_call_id": toolCallIds.toolCall07,
            "content": SETUP_MESSAGES.TOOL_CALL_07_SECRET_CIPHER_AURIST
          },
          {
            "role": "tool",
            "tool_call_id": toolCallIds.toolCall08,
            "content": SETUP_MESSAGES.TOOL_CALL_08_SECRET_CIPHER_JOSE
          }
        ],
        "tools": TOOLS_AVAILABLE,
        "stream": isStream
    ...
    describe(`${getTestTags(test.provider, test.model)} @ai: ${test.provider} (${test.model}) [text/event-stream]`, function () {
      it('stage 1: replies with the initial discovery function call', async function () {
        await withRetry(async () => {
          const events = await makeToolUseRequestStage1(proxyUrl, path, true);
      ...
      it('stage 2: we reply with consumer metadata, then the model requests function answers for consumer API usage per-minute and city weather (order-insensitive)', async function () {
        await withRetry(async () => {
          const events = await makeToolUseRequestStage2(proxyUrl, path, true, test.settings.toolCallIdPrefix as string, test.settings.toolCallIdLength as number);
          ...
          const expectedToolCalls = [
            {
              ...
              function: {
                name: "get_weather_in_city_in_centigrade_and_fahrenheit",
                arguments: JSON.parse("{\"country\": \"USA\", \"city\": \"New York\"}")
              }
            },
            {
              type: "function",
              function: {
                name: "get_weather_in_city_in_centigrade_and_fahrenheit",
                arguments: JSON.parse("{\"country\": \"Norway\", \"city\": \"Oslo\"}")
              }
            },
            {
              type: "function",
              function: {
                name: "get_multiple_consumer_api_usage_per_minute",
                arguments: JSON.parse("{\"consumers\": [{\"username\": \"JACKT\"}, {\"username\": \"AURIST\"}, {\"username\": \"JOSEP\"}]}")
              }
            },
            {
              type: "function",
              function: {
                name: "get_weather_in_city_in_centigrade_and_fahrenheit",
                arguments: JSON.parse("{\"country\": \"Spain\", \"city\": \"Madrid\"}")
              }
            }
          ];
      ...
      it('stage 3: we respond with consumer API usage per minute and city weather, then model request function answers for secret-cipher (order-insensitive)', async function () {
        await withRetry(async () => {
          const events = await makeToolUseRequestStage3(proxyUrl, path, true, test.settings.toolCallIdPrefix as string, test.settings.toolCallIdLength as number);
          ...
          const expectedToolCalls = [
            {
              ...
              function: {
                name: "secret_cipher",
                arguments: JSON.parse("{\"temperature\": 35}")
              }
            },
            {
              type: "function",
              function: {
                name: "secret_cipher",
                arguments: JSON.parse("{\"temperature\": 40}")
              }
            },
            {
              type: "function",
              function: {
                name: "secret_cipher",
                arguments: JSON.parse("{\"temperature\": 30}")
              }
            }
          ];
      ...
      it('stage 4: respond with correct secret ciphers based off local function result(s)', async function () {
        await withRetry(async () => {
          const events = await makeToolUseRequestStage4(proxyUrl, path, true, test.settings.toolCallIdPrefix as string, test.settings.toolCallIdLength as number);
    ...
    describe(`${getTestTags(test.provider, test.model)} @ai: ${test.provider} (${test.model}) [application/json]`, function () {
      it('stage 1: replies with the initial discovery function call', async function () {
        await withRetry(async () => {
          const stage1Raw = await makeToolUseRequestStage1(proxyUrl, path, false);
          const stage1 = typeof stage1Raw === 'string' ? JSON.parse(stage1Raw) : stage1Raw;
          ...
          expect(stage1!.choices[0].message.tool_calls[0].function.name, 'Should call `get_consumer_metadata_from_ids` function with an array parameter').to.equal('get_consumer_metadata_from_ids');

          const tool_1_args = JSON.parse(stage1!.choices[0].message.tool_calls[0].function.arguments);
          expect(tool_1_args).to.have.property('consumer_ids').that.is.an('array').that.is.not.empty;
          expect(tool_1_args.consumer_ids).to.have.members(['101', '102', '103'], 'Should have consumer IDs 101, 102, and 103 in the tool call args');
        }, 5)
      });
      ...
      it('stage 2: we reply with consumer metadata, then the model requests function answers for consumer API usage per-minute and city weather (order-insensitive)', async function () {
        await withRetry(async () => {
          const stage2Raw = await makeToolUseRequestStage2(proxyUrl, path, false, test.settings.toolCallIdPrefix as string, test.settings.toolCallIdLength as number);
          const stage2 = typeof stage2Raw === 'string' ? JSON.parse(stage2Raw) : stage2Raw;
      ...
      it('stage 3: we respond with consumer API usage per minute and city weather, then model request function answers for secret-cipher (order-insensitive)', async function () {
        await withRetry(async () => {
          const stage3Raw = await makeToolUseRequestStage3(proxyUrl, path, false, test.settings.toolCallIdPrefix as string, test.settings.toolCallIdLength as number);
          const stage3 = typeof stage3Raw === 'string' ? JSON.parse(stage3Raw) : stage3Raw;
      ...
      it('stage 4: respond with correct secret ciphers based off local function result(s)', async function () {
        await withRetry(async () => {
          const stage4Raw = await makeToolUseRequestStage4(proxyUrl, path, false, test.settings.toolCallIdPrefix as string, test.settings.toolCallIdLength as number);
          const stage4 = typeof stage4Raw === 'string' ? JSON.parse(stage4Raw) : stage4Raw;
