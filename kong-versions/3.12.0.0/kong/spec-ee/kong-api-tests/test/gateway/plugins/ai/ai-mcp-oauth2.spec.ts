...
describe('@ai: Gateway Plugins: AI MCP OAuth2', function () {
  ...
  before(async function () {
    const isKeycloakMCPRunning = await isContainerRunningByName('keycloak-mcp');
    if (!isKeycloakMCPRunning) {
      runDockerComposeCommand('--profile manual up -d keycloak-mcp');
  ...
  after(async function () {
    runDockerComposeCommand('rm -sf keycloak-mcp');
  });
});
