...
-- This function is used for the ace tests to enable the admin api
-- temporarily by including the modified `spec/fixtures/ace/daos.lua`
-- at the start of LUA_PATH.
local function enable_ace_admin_api()
  local temp_dir = make_temp_dir()
  local dest_dir = temp_dir .. "/kong/plugins/ace"
  assert(pl_dir.makepath(dest_dir), "failed to makepath " .. dest_dir)

  local origin_lua_path = os.getenv("LUA_PATH")
  local new_lua_path = temp_dir .. "/?.lua;" .. temp_dir .. "/?/init.lua;" .. origin_lua_path
  -- put the temp dir at first
  assert(sys.setenv("LUA_PATH", new_lua_path), "failed to set LUA_PATH env")


  assert(shell.run("cp -f spec/fixtures/ace/daos.lua " .. dest_dir), "failed to copy spec/fixtures/ace/daos.lua to " .. dest_dir)
...
return {
  ...
  enable_ace_admin_api = enable_ace_admin_api,
