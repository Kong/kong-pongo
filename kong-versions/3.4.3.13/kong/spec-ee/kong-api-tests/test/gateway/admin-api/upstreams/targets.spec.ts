...
describe('@gke: Gateway Admin API: Targets', function () {
  ...
  it('should not create target if not given target field', async function () {
    const resp = await postNegative(`${url}/${upstreamData.id}/targets`, {}, targetCreationMethod);
    logResponse(resp);
  ...
  it.skip('should not create duplicate of existing target using host:port', async function () {
    const resp = await postNegative(`${url}/${upstreamData.id}/targets`, { target: target }, 'put');
    logResponse(resp);
  ...
  it('should create target associated with upstream by upstream name', async function () {
    ...
    expect(resp.data, 'should have associated upstream').to.have.property('upstream');
    expect(resp.data.upstream.id, 'should have correct upstream id').to.equal(upstreamData.id);
  ...
  it('should edit target with PATCH using upstream id and target id', async function () {
    ...
    expect(resp.data.target, 'expect updated target in response').to.equal(updatedTarget);
  ...
  it('should edit target with PATCH using upstream id and target', async function () {
    ...
    expect(resp.status, 'should return 200 status').to.equal(200);
    expect(resp.data.id, 'expect id to be in response').to.equal(targetData.id);
    expect(resp.data.target, 'expect updated target in response').to.equal(targetData.target);
  ...
  it('should see updated target associated with upstream using upstream id', async function () {
    ...
    expect(resp.data.data[0].target, 'should have updated target').to.equal(targetData.target);
  });
  ...
  it('should edit target with PATCH using upstream name and target id', async function () {
    ...
    expect(resp.status, 'should return 200 status').to.equal(200);
    expect(resp.data.id, 'expect id to be in response').to.equal(targetData.id);
    expect(resp.data.target, 'expect updated target in response').to.equal(target);
  ...
  it('should confirm healthy status', async function () {
    ...
    const assertions = resp => {
      expect(resp.status, 'should return 200 status').to.equal(200);
      expect(resp.data.data[0].id, 'should match target id').to.equal(targetData.id);
      expect(resp.data.data[0].health, 'should show a status of HEALTHY').to.equal('HEALTHY');
      expect(resp.data.data[0].upstream.id, 'should show associated upstream in response').to.equal(upstreamData.id);
    };
  ...
  it('should confirm unhealthy status', async function () {
    ...
    const assertions = resp => {
      expect(resp.status, 'should return 200 status').to.equal(200);
      expect(resp.data.data[0].id, 'should match target id').to.equal(targetData.id);
      expect(resp.data.data[0].health, 'should show a status of UNHEALTHY').to.equal('UNHEALTHY');
      expect(resp.data.data[0].upstream.id, 'should show associated upstream in response').to.equal(upstreamData.id);
    };
