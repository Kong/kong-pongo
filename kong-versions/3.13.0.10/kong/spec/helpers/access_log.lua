...
function _M.reset_log_file(path)
  ...
  file:close()
  -- file-log writes from the nginx worker, which runs as `nginx_user` (the
  -- `kong` user by default) and may differ from the user running the test
  -- process. When the test process creates the file (e.g. via os.tmpname(),
  -- which yields a 0600 file owned by that user), the worker cannot open it
  -- for writing and the log stays empty. Make it world-writable so the worker
  -- can always append to it.
  os.execute(string.format("chmod 0666 %q", path))
end
