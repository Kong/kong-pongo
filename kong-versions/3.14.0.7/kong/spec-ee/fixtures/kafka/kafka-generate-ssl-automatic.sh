...
openssl pkcs12 -in $KEYSTORE_WORKING_DIRECTORY/$CLIENT_KEYSTORE_FILENAME -out $KEYSTORE_WORKING_DIRECTORY/combined.key -passin pass:$PASS -passout pass:$PASS -legacy
echo "Only get the key"
openssl pkcs12 -info -in $KEYSTORE_WORKING_DIRECTORY/$CLIENT_KEYSTORE_FILENAME -nodes -nocerts -passin pass:$PASS -out $KEYSTORE_WORKING_DIRECTORY/privkey.key -legacy
echo "Only get the cert chain"
openssl pkcs12 -info -in $KEYSTORE_WORKING_DIRECTORY/$CLIENT_KEYSTORE_FILENAME -nodes -nokeys -passin pass:$PASS -out $KEYSTORE_WORKING_DIRECTORY/certchain.crt -legacy
