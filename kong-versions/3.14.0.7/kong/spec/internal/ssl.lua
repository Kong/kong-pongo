...
local function handle_ssl_io(self, cb, ...)
  ...
  while true do
    ...
    if code == errors.SSL_ERROR_NONE then
      return err, nil  -- return byte count on success
...
function SSL:receive(pattern)
  ...
  while true do
    local n, err = handle_ssl_io(self, C.SSL_read, ffi.cast("void *", buf), 1024)
    if err then
    ...
    local current = ffi.string(buf, n)  -- use actual byte count
