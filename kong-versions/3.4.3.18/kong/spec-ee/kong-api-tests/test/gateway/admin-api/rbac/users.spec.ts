...
describe('@smoke: Gateway RBAC: Users', function () {
  ...
  it('should create a user', async function () {
    ...
    expect(resp.data.user_token, 'Should have encrypted user_token').to.not.contain(user1Token);
    expect(resp.data.user_token_ident, 'Should have user_token_ident').to.be.a.string;
  ...
  it('should not create a user without token', async function () {
    ...
    expect(resp.data.name, 'Should have correct error name').to.equal('schema violation');
    expect(resp.data.message, 'Should have correct message').to.contain('schema violation (user_token:');
    expect(resp.data.fields, 'Should have correct violated fields').to.haveOwnProperty(
      'user_token',
  ...
  it('should not create a user without name', async function () {
    ...
    const assertions = resp => {
      expect(resp.status, 'Status should be 400').to.equal(400);
      expect(resp.data.name, 'Should have correct error name').to.equal('schema violation');
      expect(resp.data.message, 'Should have correct message').to.contain('schema violation (name:');
      expect(resp.data.fields, 'Should have correct violated fields').to.haveOwnProperty(
        'name',
        'required field missing',
      );
  ...
  it('should not get a user by wrong id', async function () {
    const resp = await getNegative(`${url}/0cb764cd-0b2f-4956-b7f2-c3cb60c55907`);
    logResponse(resp);
  ...
  it('should update the user', async function () {
    ...
    expect(resp.data.id, 'Should have correct user id').to.equal(user1Id);
    expect(resp.data.comment, 'Should have updated comment').to.equal('A new comment');
    expect(resp.data.id, 'Should have updated user token').to.not.eq(user1Token);
  });
  ...
  it('should create a user with enabled false', async function () {
    ...
    expect(resp.data.user_token, 'Should have encrypted user_token').to.not.contain(user2Token);
  ...
  it('should get all users', async function () {
    ...
    resp.data.data.forEach(user => {
      expect(user.name, 'Should see all user names').to.equal(user.id === user1Id ? user1UpdatedName : user2Name);
    });
  });
