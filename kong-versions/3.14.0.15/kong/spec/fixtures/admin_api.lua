...
for name, dao in pairs(helpers.db.daos) do
  ...
  admin_api_as_db[name] = {
    ...
    remove = function(_, tbl)
      local res = api_send("DELETE", "/" .. admin_api_name .. "/" .. tbl.id)

      -- DELETE returns 204 No Content on success and the api_send helper returns nil in the case
      if res == nil then
        -- As our system is eventually consistent, we wait until the entity is really gone
        helpers.wait_until(function()
          local _, err = api_send("GET", "/" .. admin_api_name .. "/" .. tbl.id)
          return err and string.find(err, "404")
        end, 20, 0.5)
      end

      -- return the original response from the DELETE operation
      return res
    end,
