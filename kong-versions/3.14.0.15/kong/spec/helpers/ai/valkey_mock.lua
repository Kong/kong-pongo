...
-- connection accounting, so a test can pin that every connection the driver
-- opens is either pooled or closed, on success and on failure alike
local connections_opened = 0
local connections_released = 0
  ...
  mocker.setup(finally, {
    modules = {
      { "resty.redis.connector", {
        new = function()
          ...
          return {
            ...
            set_keepalive = function()
              ...
              connections_released = connections_released + 1
              return true
            end,
            close = function()
              connections_released = connections_released + 1
            ...
            ["scan"] = function(red, cursor, ...)
              if forced_op_errors["scan"] then
                return red:ret(nil, forced_op_errors["scan"])
              end
            ...
            ["del"] = function(red, ...)
              if forced_op_errors["del"] then
                return red:ret(nil, forced_op_errors["del"])
              end
        ...
        connection_stats = function()
          return {
            opened = connections_opened,
            released = connections_released,
          }
        end,
      } },
    }
