...
-- error messages to force per raw command, e.g. ["FT._LIST"] = "unknown command"
local forced_op_errors = {}
-- replies to force per raw command, e.g. ["FT._LIST"] = "unsupported"
local forced_op_results = {}
  ...
  mocker.setup(finally, {
    modules = {
      { "resty.redis.connector", {
        new = function()
          operation_counts.new = operation_counts.new + 1
          return {
            -- function mocks
            set_timeouts = function() end,
            connect = function(red)
              operation_counts.connect = operation_counts.connect + 1
            ...
            ["FT._LIST"] = function(red, ...)
              if select("#", ...) ~= 0 then
                return red:ret(false, "FT._LIST does not accept arguments")
              end

              if forced_op_results["FT._LIST"] ~= nil then
                return red:ret(forced_op_results["FT._LIST"])
              end

              local err = forced_op_errors["FT._LIST"] or forced_error_msg
              if err then
                return red:ret(false, err)
              end

              local names = {}
              for index_name in pairs(indexes) do
                names[#names + 1] = index_name
              end
              return red:ret(names)
            end,
            ...
            ["FT.SEARCH"] = function(red, index, ...)
              ...
              for _key, value in pairs(data) do
                ...
                if proximity_match then
                  table.insert(payloads, { "score", tostring(distance), "payload", cjson.encode({ decoded_payload.payload }) })
                end
              end
        ...
        get_operation_counts = function()
          return operation_counts
        end,
        forced_op_failure = function(op, err_msg)
          forced_op_errors[op] = err_msg
        end,
        forced_op_response = function(op, result)
          forced_op_results[op] = result
        end,
      } },
    }
  })
...
local function clear()
  ...
  indexes = {}
  ttl = {}
  operation_counts.new = 0
  operation_counts.connect = 0
  forced_op_errors = {}
  forced_op_results = {}
end
