...
describe('@smoke: Router Functionality Tests', function () {
  ...
  it('should create a route with header', async function () {
    ...
    expect(resp.data.name, 'Should have correct route name').equal(payload.name);

    expect(resp.data.headers, 'Should have correct route header key').to.have.property('testHeader');

    await wait(isHybrid ? hybridWaitTime + (isLocalDb ? 0 : hybridWaitTime) : waitTime);
  });

  it('should not create a route with duplicate name', async function () {
    const resp = await postNegative(routesUrl, routePayload, 'post', {}, { rejectUnauthorized: true });
    logResponse(resp);

    expect(resp.status, 'Status should be 409').equal(409);
    expect(resp.data.name, 'Should have correct error name').equal('unique constraint violation');
  ...
  testWrongHeaders.forEach(({ header, value }) => {
    it(`should not route a request with wrong header: ${header}:${value}`, async function () {
      ...
      expect(resp.data.message, 'Should have correct error name').equal(`no Route matched with those values`);
    });
  });

  it('should not route a request without the required header', async function () {
    const resp = await getNegative(`${proxyUrl}${routePayload.paths[0]}`, {}, {}, { rejectUnauthorized: true });
    logResponse(resp);
  ...
  regexWrongPatterns.forEach(wrongPattern => {
    it(`should not route a request with non-matching regex path: ${wrongPattern}`, async function () {
      const resp = await getNegative(`${proxyUrl}${wrongPattern}`, {}, {}, { rejectUnauthorized: true });
      logResponse(resp);
      expect(resp.status, 'Status should be 404').equal(404);
    });
  });
  ...
  it('should route the request with correct host', async function () {
    ...
    expect(resp.status, 'Status should be 200').equal(200);
    expect(resp.data.hosts[0], 'Should have correct host').to.equal(currentHost);
  ...
  it('should perform route validation when creating a duplicate route in a different workspace', async function () {
    ...
    await eventually(async () => {
      ...
      expect(respRepeat.data.message, 'Should have correct error message').to.equal(
