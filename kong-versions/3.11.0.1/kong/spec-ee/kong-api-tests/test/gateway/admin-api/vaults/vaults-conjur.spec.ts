...
import { expect, postNegative, getBasePath, Environment, logResponse, isGateway, clearAllKongResources, createGatewayService, createRouteForService, createPlugin, createConsumer, createKeyCredentialForConsumer, waitForConfigRebuild, getNegative, updateKeyCredentialForConsumer, eventually, randomString, patchPlugin, isGwHybrid, isLocalDatabase } from '@support';
import { createConjurAccount, deleteConjurAccount, loadConjurPolicy, setConjurSecret, updateConjurPolicy } from 'support/utilities/conjur-vaults';
...
describe('Vaults: Conjur', function () {
  ...
  const rotatedSecretValue = async (conjurApiKey: string, secretName: string, consumerId: string, consumerKeyId: string, forceSecretValue?: string, isBase64 = false): Promise<string> => {
  ...
  const assertBasicDetails = (resp: AxiosResponse, vaultName: string, vaultPrefix: string, conjurUrl: string, conjurAccount: string, conjurLogin: string, conjurApiKey: string) => {
  ...
  it('should create conjur vault with a valid config', async function () {
    ...
    assertBasicDetails(resp, vaultName, vaultPrefix, constants.conjur.CONJUR_URL, constants.conjur.CONJUR_ACCOUNT, constants.conjur.CONJUR_LOGIN, conjurApiKey);
  ...
  it('should cache conjur secret for the given TTL period', async function () {
    ...
    console.log(`Old secret value: ${conjurSecretValue}`);
    const rotatedKey1 = await rotatedSecretValue(conjurApiKey, conjurFirstSecretName, consumerId, consumerKeyId);
    console.log(`Old secret value: ${rotatedKey1}`);
    const rotatedKey2 = await rotatedSecretValue(conjurApiKey, conjurFirstSecretName, consumerId, consumerKeyId);
    console.log(`New secret value: ${rotatedKey2}`);
    conjurSecretValue = rotatedKey2;
    await waitForConfigRebuild();
  ...
  it('should decode secrets as base64 when setting is enabled', async function () {
    ...
    const isBase64 = true;
    await rotatedSecretValue(conjurApiKey, conjurFirstSecretName, consumerId, consumerKeyId, decodedBase64Secret, isBase64);
    await waitForConfigRebuild();
