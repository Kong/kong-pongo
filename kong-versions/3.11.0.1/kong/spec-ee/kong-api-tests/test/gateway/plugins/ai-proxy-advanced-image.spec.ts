...
describe('@weekly: Gateway Plugins: AI Proxy Image Generation and Edit Test', function () {
  const aws_region_image = 'us-east-1';
  const imagePath = path.resolve(__dirname, '../../../support/data/ai/image_edit.png');
  const maskPath = path.resolve(__dirname, '../../../support/data/ai/mask.png');
  ...
  */
  function validateImageGenerationResponse(resp: any, provider = 'unknown') {
    ...
    if (resp.data.usage && resp.data.usage.output_tokens) {
      console.log(`Output tokens: ${resp.data.usage.output_tokens}`);
  ...
  */
  ...
  it.skip('should patch update AI proxy advanced plugin with awd bedrock image edit', async function () {
    const geminiImagePayload = _.cloneDeep(pluginPayload);
    geminiImagePayload.service.id = serviceId;
  ...
  //skipping this test as it is not supported in the current version of the plugin AG-350
  it.skip('should be able to send prompt to bedrock image model via route and to edit image', async function () {
