...
local function database_has_procedure(state, arguments)
  local procedure_name = arguments[1]
...
say:set("assertion.database_has_procedure.positive", "Expected database to have procedure %s")
say:set("assertion.database_has_procedure.negative", "Expected database not to have procedure %s")
assert:register("assertion", "database_has_procedure", database_has_procedure, "assertion.database_has_procedure.positive", "assertion.database_has_procedure.negative")
...
-- we can't just import "kong.tools.string" because older versions of Kong may not have this module
...
do
  local function get_version_num(v1, v2)
    if v2 then
      assert(#(split_by_dot(v2)) == #(split_by_dot(v1)), string.format("different version format: %s and %s", v1, v2))
    end
