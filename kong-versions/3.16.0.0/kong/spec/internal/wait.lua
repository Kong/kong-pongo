...
do
  -- The test nginx template statically claims the mock_upstream ports, but it
  -- only binds them after Kong starts. A port picked during setup passes the
  -- `netstat` check while Kong is still down, so it can be handed out and then
  -- collide once Kong binds the same port. Mark these ports as used up front so
  -- they are never returned. These are the only static ports inside the random
  -- range below.
  local USED_PORTS = {
    [CONSTANTS.MOCK_UPSTREAM_PORT]            = true,
    [CONSTANTS.MOCK_UPSTREAM_SSL_PORT]        = true,
    [CONSTANTS.MOCK_UPSTREAM_STREAM_PORT]     = true,
    [CONSTANTS.MOCK_UPSTREAM_STREAM_SSL_PORT] = true,
  }
...
-- @tparam[opt] boolean opts.stream_enabled to enable stream module
...
-- @tparam[opt] (EE Only) boolean opts.new_workspace to create a new workspace for checking
...
local function wait_for_all_config_update(opts)
  ...
  -- XXX EE [[
  local new_workspace = opts.new_workspace or false
  ...
  local host_on_route = "really.really.really.really.really.really.really.mocking.route.test"
  ...
  if new_workspace then
    local res = assert(call_admin_api("POST",
                                  "/workspaces",
                                  { name = "really-really-really-really-really-really-really-mocking-workspace" },
                                  201, headers))
  ...
  res = assert(call_admin_api("POST",
                       ...
                       { paths = { route_path }, hosts= { host_on_route }, strip_path = true, path_handling = "v0", protocols = { "http", "https" }, },
                       201, headers))
  route_id = res.id
  ...
  -- XXX EE [[
  if workspace_name then
    call_admin_api("DELETE", "/workspaces/" .. workspace_name .. "?cascade=true", nil, 204, headers)
  end
  -- XXX EE ]]
...
local function reload_kong(cmd, env, wait_opts)
  local nginx_conf = ""
  if env and env.nginx_conf then
    nginx_conf = " --nginx-conf " .. env.nginx_conf
  end

  local workers = get_kong_workers()

  local ok, err = shell.kong_exec(cmd .. (nginx_conf or ""), env)
  if ok then
    wait_until_no_common_workers(workers, 1, wait_opts or { timeout = 30 })
  end
  return ok, err
end
-- XXX EE ]]
...
--- Waits until no TCP socket is listening on any of the given ports.
-- Intended as a pre-start guard so a spec can defend itself against a leaked
-- Kong process from a prior iteration that still holds a shared port (e.g. the
-- mock upstream's 15555). Without this the next `start_kong` surfaces a
-- confusing `nginx: [emerg] bind() to 127.0.0.1:<port> failed (98: Address
-- already in use)`; this turns it into a clear "port still has a listener"
-- timeout and gives the leaked process time to exit.
--
-- NOTE: this is a regular Lua function, not a Luassert assertion.
-- @function wait_until_no_listen
-- @param ports a single port (number/string) or a list of ports
-- @param timeout (optional) maximum time to wait, defaults to 30
-- @return true when all ports are free, throws on timeout
local function wait_until_no_listen(ports, timeout)
  if type(ports) ~= "table" then
    ports = { ports }
  end
  assert(#ports > 0, "wait_until_no_listen: ports must not be empty")
  timeout = timeout or 30

  -- validate every port as an integer in range before building the pattern.
  -- This both rejects bad input early and ensures the values interpolated into
  -- the grep pattern / shell command below are plain digits, so they cannot
  -- carry regex or shell metacharacters.
  local port_strs = {}
  for i, port in ipairs(ports) do
    local n = tonumber(port)
    assert(n and n == math.floor(n) and n >= 1 and n <= 65535,
           "wait_until_no_listen: invalid port: " .. tostring(port))
    port_strs[i] = tostring(n)
  end

  -- match any of the ports as a listening local address (":<port> ") so a
  -- single `netstat` runs per poll regardless of how many ports we guard
  -- (mirrors the netstat check in `get_available_port`)
  local pattern = ":(" .. table.concat(port_strs, "|") .. ") "

  wait_until(function()
    -- grep exit status: 0 = a listener was found (at least one port still
    -- occupied), 1 = clean no-match (all free), >1 = execution error. Inspect
    -- the explicit status so a netstat/grep failure is not silently treated as
    -- "free", which would make the guard pass without actually checking.
    local ok, _, stderr, _, status =
      shell.run("netstat -lnt | grep -E \"" .. pattern .. "\" > /dev/null", nil, 0)
    if ok then
      return false -- listener present, keep waiting
    end
    -- grep exits 1 on a clean no-match (ports free). But the status of a
    -- pipeline reflects only its last command (grep), so an upstream `netstat`
    -- failure can still surface as status==1 -- it just feeds grep no input.
    -- Treat any stderr output as an execution error so we don't silently skip
    -- the check when the command itself is broken.
    assert(status == 1 and (stderr == nil or stderr == ""),
           "wait_until_no_listen: netstat/grep failed (status " .. tostring(status) ..
           "): " .. tostring(stderr))
    return true -- grep cleanly reported no listener
    -- poll at 0.5s: each iteration forks `netstat | grep`, and a freed port is
    -- a seconds-scale event, so the default 0.05s step would just fork-storm.
  end, timeout, 0.5,
     "timed out waiting for port(s) to be free: " .. table.concat(port_strs, ", "))
...
return {
  ...
  wait_until_no_listen = wait_until_no_listen,
