...
> end
        ...
        location = /ws/reject-content-length {
            content_by_lua_block {
                local mu = require "spec.fixtures.mock_upstream"
                return mu.reject_web_socket_handshake_with_content_length(
                    "{\"message\":\"content-length handshake rejection\"}"
                )
            }
        }

        location = /ws/reject-content-length-large {
            content_by_lua_block {
                local mu = require "spec.fixtures.mock_upstream"
                ngx.status = ngx.HTTP_BAD_REQUEST
                return mu.send_text_response(string.rep("x", 64 * 1024 + 17), "text/plain")
            }
        }

        location = /ws/reject-forbidden {
            content_by_lua_block {
                local mu = require "spec.fixtures.mock_upstream"
                return mu.reject_web_socket_handshake_with_content_length(
                    "{\"message\":\"forbidden\"}",
        ...
        location = /ws/reject-chunked-trailer {
            add_trailer X-Mock-WebSocket-Trailer "mock trailer value" always;

            content_by_lua_block {
                local mu = require "spec.fixtures.mock_upstream"
                return mu.reject_web_socket_handshake_chunked(
                    "{\"message\":\"chunked trailer handshake rejection\"}",
                    nil,
                    { ["Trailer"] = "X-Mock-WebSocket-Trailer" }
                )
            }
        }
        ...
        location = /echo_conn {
            ...
            content_by_lua_block {
                ngx.say(string.format(
                    '{"connection": "%s", "connection_requests": "%s"}',
                    ngx.var.connection,
