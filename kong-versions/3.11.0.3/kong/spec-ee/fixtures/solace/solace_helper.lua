...
function _M.start()
  ...
  -- Wait for solace-setup to complete
  local timeout = 60
  local count = 0
  while true do
    local result, check_err = execute_command("docker ps --filter 'name=" .. SOLACE_PROJECT_NAME .. "-setup' --filter 'status=running' -q")
    if check_err then
      pl_path.chdir(original_dir)
      return nil, "Failed to check setup container status: " .. check_err
    end

    if not result or result:match("^%s*$") then
      -- Setup container is no longer running (completed)
      break
    end
    ...
    if count >= timeout then
      pl_path.chdir(original_dir)
      return nil, "Timeout waiting for setup container to complete"
    end
