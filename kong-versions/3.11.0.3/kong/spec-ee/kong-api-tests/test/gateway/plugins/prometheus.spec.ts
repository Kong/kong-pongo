...
describe('@oss: Gateway Plugins: Prometheus', function () {
  ...
  before(async function () {
    const isPrometheusRunning = await isContainerRunningByImage('prometheus');
    if(!isPrometheusRunning){
      runDockerComposeCommand('--profile manual up -d prometheus');
    }
  ...
  //add test to cover ADEO issue KAG-6793
  it('should see kong_data_plane_config_hash metric after enabling prometheus plugin', async function () {
    await eventually(async () => {
      if(isGwHybrid()) {
        const kongData = await getAllMetrics();
        expect(kongData.includes('kong_data_plane_config_hash{node_id='), 'should see kong_data_plane_config_hash in control plane metrics').to.be.true
      }
    });
  });
  ...
  after(async function () {
    runDockerComposeCommand('rm -sf prometheus');
