...
for (const test of tests) {
  ...
  describe(`@ai: Gateway Plugins: ${test.provider} (${test.model}) AI Proxy Advanced Structured Output Tools Use application/json`, function () {
    ...
    const path = '/ai_proxy_advanced_structured_output_test';
    ...
    it('should create AI proxy plugin with one target', async function () {
      ...
      //     "config": {
      //       "body_filter": [
      //         "kong.log.warn('--> FROM LLM: ', ngx.arg[1])"
      //       ]
      //     }
      //   },
      //   validateStatus: null
      ...
      //     "config": {
      //       "body_filter": [
      //         "kong.log.warn('--> TO USER: ', require('kong.tools.gzip').inflate_gzip(ngx.arg[1]))"
      //       ]
      //     }
      //   },
      //   validateStatus: null
      ...
      //     "config": {
      //       "access": [
      //         "kong.log.warn('--> TO LLM: ', kong.request.get_raw_body()"
      //       ]
      //     }
      //   },
      //   validateStatus: null
    ...
    it('structured output with object type root', async function () {
      await withRetry(async () => {
        const resRaw = await makeStructuredOutputRequestObjectRoot(proxyUrl, path);
    ...
    it('structured output with array type root', async function () {
      ...
      await withRetry(async () => {
        const resRaw = await makeStructuredOutputRequestArrayRoot(proxyUrl, path);
        const res = typeof resRaw === 'string' ? JSON.parse(resRaw) : resRaw;
    ...
    it('structured output with extreme complex schema', async function () {
      ...
      await withRetry(async () => {
        const resRaw = await makeStructuredOutputRequestComplexSchema(proxyUrl, path);
        const res = typeof resRaw === 'string' ? JSON.parse(resRaw) : resRaw;
    ...
    it('structured output with empty array', async function () {
      await withRetry(async () => {
        const resRaw = await makeStructuredOutputRequestContainsEmptyArray(proxyUrl, path);
        const res = typeof resRaw === 'string' ? JSON.parse(resRaw) : resRaw;
