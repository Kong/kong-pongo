...
local function push_data(config, batch)
  ...
  elseif signal_type == "profiles" then
    host = config.profiles_host
    port = config.profiles_port
  else
...
function _M:ws_client_frame(config)
  ...
  elseif path == "/v1/analytics/session-profiles" then
    signal_type = "profiles"
    decoded = pb.decode("kong.model.observability.ProfileRequest", unzipped)
  else
    return
  end
  ...
  if (not decoded.resource_spans or #decoded.resource_spans == 0) and
     (not decoded.resource_logs or #decoded.resource_logs == 0) and
     (not decoded.resource_profiles or #decoded.resource_profiles == 0) then
    return
  end
  ...
  local handler_conf = {
    ...
    profiles_host = config.profiles_host,
    profiles_port = config.profiles_port,
  }
