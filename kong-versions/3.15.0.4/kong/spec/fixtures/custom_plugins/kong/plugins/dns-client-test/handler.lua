...
-- Atomic, lock-free counting barrier shared across nginx workers. Each worker
-- bumps the counter and waits until every worker has done the same, so phase 1
-- and phase 2 run in lockstep regardless of how staggered the workers'
-- init_worker timers fire (the original source of flakiness on slow CI hosts).
local SHM = ngx.shared.kong

local function barrier(key)
  SHM:incr(key, 1, 0)
  local total = ngx.worker.count()
  local deadline = ngx.now() + 10
  while (SHM:get(key) or 0) < total do
    if ngx.now() >= deadline then
      log(ERR, PRE, "barrier timeout waiting for ", key)
      return
    end
    ngx.sleep(0.01)
  end
...
local function test()
  ...
  phase = "first:"
  barrier("dns_ipc_test:phase1_start")
  ...
  phase = "stale:"
  barrier("dns_ipc_test:phase2_start")
  local answers = cli:_resolve(host)
  log(ERR, PRE, phase, "answers:", answers[1].address)
