...
local function get_patched_helpers(strategy, deploy, rpc, rpc_sync, opts)
  ...
  function _M.start_kong(env, tables, preserve_prefix, fixtures)
    ...
    if deploy == "traditional" then
      ...
      local hybrid_envs = {
        ...
        internal_cluster_rpc = rpc,
        incremental_sync = rpc_sync,
        prefix = "",
        role = "",
        cluster_listen = "",
  ...
  function _M.wait_for_all_config_update(wait_opts)
    if strategy ~= "off" then
      ...
      -- XXX EE [[
      if opts.new_workspace_for_waiting_config_update then
        copied_opts.new_workspace = true
      end
      -- XXX EE ]]
...
--   - new_workspace_for_waiting_config_update: (EE Only) (boolean) if true, waiting config update will perform in a new workspace
