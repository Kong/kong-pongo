...
function _M.new(db)
  ...
  res.partials = new_blueprint(db.partials, function (overrides)
    return {}
  end)
  ...
  res.custom_plugins = new_blueprint(db.custom_plugins, function(overrides)
    ...
    if not custom_plugin_enabled then
      rawset(kong.configuration, "custom_plugin_streaming_enabled", true)
    end
    ...
    if not custom_plugin_enabled then
      rawset(kong.configuration, "custom_plugin_streaming_enabled", false)
    end
    if is_cli then
      ngx.IS_CLI = true
    end
  ...
  res.cloned_plugins = new_blueprint(db.cloned_plugins, function(overrides)
    return overrides or {}
  end, function()
    local is_cli = ngx.IS_CLI
    if is_cli then
