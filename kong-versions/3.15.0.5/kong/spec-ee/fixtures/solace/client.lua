...
local DEFAULT_RDP_READY_TIMEOUT = tonumber(os.getenv("KONG_SPEC_TEST_SOLACE_RDP_READY_TIMEOUT")) or 30

local KEYCLOAK_HOST = "solace-keycloak"
local KEYCLOAK_PORT = "8080"

local keycloak_api = keycloak_api.new({
  host_name = KEYCLOAK_HOST,
  port = KEYCLOAK_PORT,
})
...
function _M.new(config)
  ...
  self.keycloak_config = keycloak_api.config
  ...
  self.rdp = {
    ...
    create_client = function(name, queue_name, options)
      ...
      local ready_timeout = tonumber(options.ready_timeout) or DEFAULT_RDP_READY_TIMEOUT
      ...
      assert.eventually(function()
        ...
        if not stats then
          return false, "Failed to get RDP stats: " .. (stats_err or "unknown error")
        end
        return stats.enabled and stats.up
      end).with_timeout(ready_timeout).is_truthy("expected RDP to be enabled and up.")
  ...
  self.oauth = {
    ...
    create_profile = function(profile_name, options)
      ...
      local keycloak_config = self.keycloak_config

      assert(keycloak_config.client_id, "Keycloak client_id must be provided in configuration")
      assert(keycloak_config.client_secret, "Keycloak client_secret must be provided in configuration")
      assert(keycloak_config.issuer, "Keycloak issuer URL must be provided in configuration")
      ...
      local oauth_profile_config = {
        ...
        endpointDiscovery = keycloak_config.issuer_discovery,
        endpointJwks = fmt("%s/protocol/openid-connect/certs", keycloak_config.issuer),
      ...
      local res, err = handle_request("POST", oauth_profile_url, oauth_profile_config)
      if not res then
        error("Failed to create OAuth profile: " .. (err or "unknown error"))
      end
      return cjson.decode(res.body)
    end,
  ...
  self.keycloak = {
    ...
    -- The test runner can reach Keycloak through the host-mapped token endpoint.
    -- Use direct HTTP requests with retries so CI readiness issues surface clearly.
    get_token = function(username, password, options)
      options = options or {}

      local keycloak_host = options.host or self.keycloak_config.host
      local client_id     = options.client_id     or self.keycloak_config.client_id
      local client_secret = options.client_secret or self.keycloak_config.client_secret
      local realm         = options.realm         or self.keycloak_config.realm
      local token_type    = options.token_type or "access_token"
      -- Construct docker exec command
      local docker_cmd = string.format(
        'docker exec -i solace curl -s -X POST "http://%s/realms/%s/protocol/openid-connect/token" ' ..
