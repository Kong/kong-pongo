...
-- in case of off strategy, we need to use the shadow_db
_G.kong.lmdb = require("kong.db.strategies.off.shadow_db").new()
_G.kong.lmdb:open()
...
local function clear_cache_on_file_end(file)
  if _G.kong and
    ...
    _G.kong.cache.mlcache.lru
  then
    -- note that this function intends to clear the cache_queue
    -- and the free_queue to release memory, but this cause severe
    -- flainess and crashes the tests;
    -- I (@StarlightIbuki) here make it to only
    -- reset the lru object. This way the logic is correct
    -- but still the memory issue is not resolved.
    -- check KAG-6941
...
--   config = {},
-- }
local function get_db_utils(strategy, tables, plugins, vaults, skip_migrations, expand_foreigns, preserve_nulls)
  ...
  if strategy ~= "off" then
    ...
    bp = assert(dc_blueprints.new(db, expand_foreigns, preserve_nulls))
    dcbp = bp
  end
