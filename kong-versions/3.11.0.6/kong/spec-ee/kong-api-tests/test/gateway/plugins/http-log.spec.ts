...
describe('@oss: Gateway Plugins: http-log', function () {
  ...
  before(async function () {
    ...
    if (!checkForArm64()) {
      await checkOrStartServiceContainer('splunk');

      //Need wait the splunk server is ready
  ...
  context('http-log with http log serveer', function () {
    ...
    it('should create http-log plugin with http-log endpoint', async function () {
      ...
      expect(resp.data.config.http_endpoint, 'Should have correct http_endpoint').to.eq(httpLogServerUrl);
    ...
    it('should send http request logs to http_endpoint', async function () {
      ...
      await eventually(async () => {
        ...
        expect(serverLogs[0]?.reqMethod, 'Should use POST method to log request data').to.eq('POST');
      });
    });
    ...
    it('should send http request random header details to http_endpoint', async function () {
      ...
      await eventually(async () => {
        ...
        expect(requestDetails?.request.method, 'Should see correct kong request method in logs').to.eq('GET');
        expect(requestDetails?.request.headers.test, 'Should see correct kong request header in logs').to.eq('test');
        expect(requestDetails?.route.paths[0], 'Should see correct kong request route path in logs').to.eq(path);
        expect(requestDetails?.service.path, 'Should see correct kong request service path in logs').to.eq('/anything');
      });
    });
    ...
    pluginConfigHeaders.forEach(pluginConfigHeader => {
      it(`should patch the http-log plugin method to ${pluginConfigHeader}`, async function () {
        ...
        expect(resp.status, 'Status should be 200').to.equal(200);
        expect(resp.data.config.method, 'Should see correct patched method').to.eq(pluginConfigHeader);
      ...
      it(`should see request logs in http_endpoint with the new ${pluginConfigHeader} method`, async function () {
        ...
        await eventually(async () => {
          ...
          expect(serverLogs?.pop()?.reqMethod, `Should use ${pluginConfigHeader} method to log request data`).to.eq(
            pluginConfigHeader,
          );
        });
      });
    ...
    it('should send resolved custom_fields_by_lua value to http_endpoint', async function () {
      ...
      expect(resp.data.config.custom_fields_by_lua.kong, 'Should see correct patched method').to.eq(
        `return 'http-log plugin api test'`,
      );
      ...
      await eventually(async () => {
        ...
        expect(requestDetails?.kong, 'Should see return value of custom_fields_by_lua').to.eq(
          'http-log plugin api test',
        );
      });
    });
    ...
    it.skip('should see http-log plugin X-header log in http_endpoint', async function () {
      ...
      expect(
        logHeaders?.some(headerObject => {
          return headerObject.name === customHeaderName.toLowerCase() && headerObject.value === 'test2';
        }),
  ...
  if (!checkForArm64()) {
    context('http-log with Splunk', function () {
      it('should create http-log plugin with http-log endpoint about Splunk server', async function () {
        ...
        logResponse(resp);
        expect(resp.status, 'Status should be 200').to.equal(200);
        expect(resp.data.config.http_endpoint, 'Should have correct http_endpoint').to.eq(splunkServerInfo.url);
        pluginId = resp.data.id;
        await waitForConfigRebuild();
      });
  ...
  after(async function () {
    ...
    if (!checkForArm64()) {
      await deleteSplunkServerHttpLogs(); //Note: Run the step only on non-ARM architecture, as the Splunk server is only supported on this architecture.
      await stopContainerByName('splunk');
    }
  });
});
