...
//  unskip when tests are fixed - (isHybrid && !isPackageTest ? describe : describe.skip)
//  https://konghq.atlassian.net/browse/KAG-7833
(isHybrid && !isPackageTest ? describe : describe.skip)('@smoke @gke: DP Resilience test for GCP', function () {
  ...
  it('should bring down kong cp with no issue', async function () {
    if (process.env.GKE!=='true') {
        runDockerContainerCommand('kong-cp', 'stop');
    } else {
        ...
        try {
            executeTerraformCommand('plan -out gwgkedeploy.plan -input=false');
            executeTerraformCommand('apply -auto-approve gwgkedeploy.plan');
          } catch (error) {
            throw new Error(
  ...
  it('should bring up kong-dp2 with cluster import and proxy works', async function () {
    if (process.env.GKE!=='true') {
        runDockerComposeCommand('up kong-dp2 -d');
    } else {
        // Execute the kubectl portforward command for kong-dp2
        try {
            kubectlWaitPod('kong', 'kong-dp2-kong');
            kubectlPortForward('kong', 'kong-dp2-kong', '8010:8010 8453:8453');
          } catch (error) {
            throw new Error(
  ...
  it('should bring up kong-dp3 with cluster export and proxy stops', async function () {
    if (process.env.GKE!=='true') {
        runDockerComposeCommand('up kong-dp3 -d');
    } else {
        // Execute the kubectl portforward command for kong-dp3
        try {
            kubectlWaitPod('kong', 'kong-dp3-kong');
            kubectlPortForward('kong', 'kong-dp3-kong', '8020:8020 8463:8463');
          } catch (error) {
            throw new Error(
              `Something went wrong during executing the kubectl portforward command: ${error}`
            );
  ...
  it('should bring up kong-dp4 with cluster export and proxy works', async function () {
    if (process.env.GKE!=='true') {
        runDockerComposeCommand('up kong-dp4 -d');
    } else {
        // Execute the kubectl portforward command for kong-dp4
        try {
            kubectlWaitPod('kong', 'kong-dp4-kong');
            kubectlPortForward('kong', 'kong-dp4-kong', '8030:8030 8473:8473');
          } catch (error) {
            throw new Error(
              `Something went wrong during executing the kubectl portforward command: ${error}`
            );
  ...
  it('should bring back kong cp and kong-dp ', async function () {
    if (process.env.GKE!=='true') {
        ...
        stopAndRemoveMultipleContainers(servicesToDelete)
        ...
        await eventually(async () => {
            ...
            while (!success) {
                try {
                    executeTerraformCommand('destroy -auto-approve');
                    executeTerraformCommand('init -input=false');
                    executeTerraformCommand('plan -out gwgkedeploy.plan -input=false');
                    executeTerraformCommand('apply -auto-approve gwgkedeploy.plan');
                    kubectlWaitPod('kong', 'kong-cp-kong');
                    kubectlWaitPod('kong', 'kong-dp1-kong');
                    kubectlWaitPod('kong', 'redis')
                    kubectlPortForward('kong', 'kong-dp1-kong', '8000:8000 8443:8443');
                    // add additional wait time for kong-cp to be ready to avoid flakiness due to kubernetes API delay
                    await wait(5000); // eslint-disable-line no-restricted-syntax
                    kubectlPortForward('kong', 'kong-cp-kong', '8001:8001 8002:8002');
                    await wait(5000); // eslint-disable-line no-restricted-syntax
                    kubectlPortForward('kong', 'redis', '6379:6379');
                    success = true;
                } catch (error) {
                    console.log(`Something went wrong during executing the terraform command: ${error}`);
                }
    ...
    // Since we restarted the kong containers, we need to check/repost license again to avoid the afterAll hook failure
    if (isCI() && !isKongOSS()) {
