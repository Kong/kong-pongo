...
function RedisUser:access(conf)
  ...
  if not red or err then
    return kong.response.exit(500, {message = "Redis not connected. Reason " .. err, config = conf })
  end

  local value_to_set = kong.request.get_header(conf.header_name)
  if not value_to_set then
    return kong.response.exit(400, { message = "Redis value to set not present in header: " .. conf.header_name, config = conf })
  end

  red:set(conf.redis_key, value_to_set)

  return kong.response.exit(200, { message = "Ok good", config = conf})
end
