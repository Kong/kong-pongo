...
describe('Gateway Admin API: TCP Route Tests', function () {
  ...
  const cpStreamPort1 = 7000;
  const streamListenPort1 = isGwHybrid() ? 7001 : cpStreamPort1;
  ...
  before(async function () {
    ...
    if (isGwHybrid()) {
      await resetGatewayContainerEnvVariable(
        {
          KONG_STREAM_LISTEN: `0.0.0.0:${streamListenPort1},0.0.0.0:${streamListenPort2}`,
        },
        'kong-dp1',
      );
    }
    ...
    const route = await createRouteForService(serviceId, null, {
      paths: null,
      protocols: ['tcp'],
      destinations: [{ port: streamListenPort1 }],
    });
    routeId = route.id;
  ...
  it('should allow when source IP and dest IP are unrestricted', async function () {
    await eventually(async () => {
      const response = await sendTcpRequest(streamListenHost, streamListenPort1, message);
      expect(response, 'Status should be "[REPLY]: hello kong"').equal(`[REPLY]: ${message}`);
    });

    // //Covers KAG-7298 and KAG-7411; steps are skipped as KAG-7411 is not fixed yet
    // const currentLogs = getGatewayContainerLogs(getKongContainerName(), 50, 'error');
  ...
  it('should return error while connection is broken', async function () {
    await eventually(async () => {
      const promise = new Promise<void>((_, reject) => {
        const client = net.createConnection({ host: '127.0.0.1', port: streamListenPort1 });
  ...
  it('should allow when source IP is unrestricted and multi dest ports are set', async function () {
    await patchRoute(routeId, {
      destinations: [{ port: streamListenPort1 }, { port: streamListenPort2 }],
    });
  ...
  it('should block when source IP is unrestricted and dest IP mismatches the TCP server', async function () {
    await patchRoute(routeId, {
      destinations: [{ ip: invalidIp, port: streamListenPort1 }],
    });
    await waitForConfigRebuild();
  ...
  if (isGwHybrid()) {
    it('should gracefully fail TCP request to kong-cp in hybrid mode without panic', async function () {
      await patchRoute(routeId, {
        destinations: [{ port: cpStreamPort1 }],
      });

      await eventually(async () => {
        try {
          const response = await sendTcpRequest(streamListenHost, cpStreamPort1, message);
          expect(response).to.equal(``);
        } catch (error) {
          if (error instanceof Error) {
            // The test is considered passed if the connection is reset.
            expect(error.message).to.include('ECONNRESET');
          } else {
            console.error('Caught non-Error object:', error);
            expect.fail(`Unexpected error type: ${typeof error}, value: ${JSON.stringify(error)}`);
          }
        }
      });
