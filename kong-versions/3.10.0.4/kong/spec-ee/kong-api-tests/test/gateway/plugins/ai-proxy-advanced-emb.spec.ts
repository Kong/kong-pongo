...
describe('Gateway Plugins: AI Proxy Advanced Embedding Test', function () {
  ...
  type EmbeddingProvider = 'openai' | 'azure' | 'bedrock';

  // Use typed keys
  const providers: EmbeddingProvider[] = ['openai', 'azure', 'bedrock'];
  ...
  async function sendEmbeddingsRequest(url: string, text: string) {
