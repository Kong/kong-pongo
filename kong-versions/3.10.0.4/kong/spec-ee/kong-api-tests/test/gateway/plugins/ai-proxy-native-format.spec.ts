...
describe('Gateway Plugins: AI Proxy native format test', function () {
  ...
  function parseNativeResponse(resp: any, variant: string): string {
    ...
    if (variant === 'gemini') {
      if (resp.candidates && resp.usageMetadata && resp.usageMetadata.promptTokenCount && resp.usageMetadata.candidatesTokenCount && resp.usageMetadata.candidatesTokensDetails) {
        expect(resp, 'Response should have candidates property').to.have.property('candidates');
        expect(resp.candidates, 'candidates should be an array').to.be.an('array');
        expect(resp.candidates, 'candidates array should not be empty').to.not.be.empty;
        expect(resp, 'Response should have usageMetadata property').to.have.property('usageMetadata');
        expect(resp.usageMetadata, 'Usage should be an object').to.be.an('object');
        expect(resp.usageMetadata.promptTokenCount, 'Input tokens should be greater than 1').to.be.greaterThan(1);
        expect(resp.usageMetadata.candidatesTokenCount, 'Output tokens should be greater than 1').to.be.greaterThan(1);
        expect(resp.usageMetadata.candidatesTokensDetails, 'candidates should be an array').to.be.an('array');
        return resp.candidates[0].content.parts[0].text;
      }
      throw new Error(`Invalid response structure for Gemini:', ${JSON.stringify(resp)}`);
      
    } else if (variant === 'bedrock') {
      if (resp.output && resp.output.message && resp.output.message.content && resp.metrics && resp.usage) {
        expect(resp.metrics, 'metrics should be an object').to.be.an('object');
        expect(resp.metrics.latencyMs, 'Metrics latency Ms should be greater than 1').to.be.greaterThan(1);
        expect(resp.usage, 'usage should not be an object').to.be.an('object');
        expect(resp.usage.outputTokens, 'Output tokens should be greater than 1').to.be.greaterThan(1);
        expect(resp.usage.inputTokens, 'Input tokens should be greater than 1').to.be.greaterThan(1);
