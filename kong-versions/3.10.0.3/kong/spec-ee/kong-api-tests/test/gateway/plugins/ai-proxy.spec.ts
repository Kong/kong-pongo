...
describe('Gateway Plugins: AI Proxy', function () {
  ...
  function evaluateResponse(resp, expectedProvider, expectedModel, type = 'chat') {
    ...
    // Check if the provider is 'anthropic' and skip id property validation
  ...
  providers.forEach((provider) => {
    ...
    it(`should be able to send properly formatted chat message to ${provider.variant} provider and chat model ${provider.chat.model} via route`, async function () {
      ...
      await retryRequest(req, assertions, 30000, 3000);
    ...
    if (provider.completions.model) {
      ...
      it(`should be able to send message to ${provider.variant} AI model ${provider.completions.model} via route with completions route type`, async function () {
        ...
        logResponse(resp)
        expect(resp.status, 'Should have 200 status code').to.equal(200)
        evaluateResponse(resp, provider.variant, provider.completions.model, 'completions')
      })
    ...
    if (provider.image.model) {
      ...
      it.skip(`should be able to send image to ${provider.variant} AI model ${provider.image.model} via route`, async function () {
        ...
        logResponse(resp)
        expect(resp.status, 'Should have 200 status code').to.equal(200)
        evaluateResponse(resp, provider.variant, provider.image.model)
    ...
    if (provider.image_generation.model) {
      ...
      it(`should be able to send prompt to ${provider.variant} image generation model ${provider.image_generation.model} via route and receive image url`, async function () {
        ...
        logResponse(resp)
        evaluateResponse(resp, provider.variant, provider.image_generation.model, 'image_generation')
        await polly.stop()
      })
    }
    ...
    it(`should be able to send message to ${provider.variant} AI proxy plugin via route with different custmoized model name`, async function () {
      ...
      const makeRequest = (model: string) => axios({
        method: 'post',
        url: `${proxyUrl}${path}`,
        data: {
          messages: [{
      ...
      const assertModelInHeader = (resp, expectedModel: string) => {
        logResponse(resp);
        if (resp.status === 504) {
          const msg = resp.data?.message || '';
          if (msg.includes('The upstream server is timing out')) {
          ...
          evaluateResponse(resp, provider.variant, provider.chat.model);
          expect(resp.headers['x-kong-llm-model'], `Response should use model "${expectedModel}" as specified in request`).to.include(expectedModel);
        } else {
          expect.fail(`Unexpected response status: ${resp.status}`);
        }
      };
      ...
      for (const model of models) {
        const resp = await makeRequest(model);
        assertModelInHeader(resp, model);
      }
