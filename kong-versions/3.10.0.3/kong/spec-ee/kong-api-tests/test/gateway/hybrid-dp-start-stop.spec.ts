...
(isHybrid ?  describe : describe.skip)('Start and Stop DP', function () {
  const workspaceName = 'start-stop-dp';
  ...
  it('stop data plane container and delete created entities', async function () {
    ...
    await runDockerContainerCommand(dataPlane1, 'stop');
    await runDockerContainerCommand(dataPlane1, 'container wait');
  ...
  it('recreate intities using the same ids and start data plance container', async function () {
    ...
    await runDockerContainerCommand(dataPlane1, 'start');
  ...
  after(async function () {
    ...
    try {
      ...
      const containerStatus = JSON.parse(await runDockerContainerCommand('kong-dp1', 'inspect'));
      ...
      if (!isHealthy) {
        ...
        await runDockerContainerCommand('kong-dp1', 'restart');
      } else {
