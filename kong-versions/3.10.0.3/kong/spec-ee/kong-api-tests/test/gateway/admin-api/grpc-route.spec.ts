/*
  ...
  - Regex path routing
  Uncovered scenarios:
  - Regex path with priority (No plugin support for gRPC transformation blocks the verification)
  ...
  context('gRPC Route with Existing and Non-existing Path', function () {
    ...
    it('should return 1 gRPC request matched gRPCs route', function (done) {
      client.makeUnaryRequest(
        ...
        err => {
          if (err) {
            console.error('Something went wrong when sending gRPC request with gRPCs route:', err);
