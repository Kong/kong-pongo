...
export const waitForConfigRebuild = async (options: any = {}) => {
  ...
  // send request to route before the plugin is created to ensure it returns 404
  const resp = await getNegative(`${proxyUrl}${routePath}`, options?.proxyReqHeader);
  expect(resp.status, 'waitForConfigRebuild - expecting new entities to be active').to.equal(404);
