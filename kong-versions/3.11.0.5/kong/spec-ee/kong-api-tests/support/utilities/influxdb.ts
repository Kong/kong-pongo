 ...
 * @returns {object}
 */
export const getAllDataFromTargetTagOrField = async (indexOfEntry: number): Promise<object> => {
  return await influx.query(`select * from ${indexOfEntry}`);
};
 ...
 * @returns {object}
 */
export const executeCustomQuery = async (customQuery: string): Promise<object> => {
  return await influx.query(customQuery);
};

/**
 * Delete all data from kong_request meaasurements
 */
export const deleteAllDataFromKongRequest = async (): Promise<void> => {
 ...
 */
export const deleteAllDataFromKongDatastoreCache = async (): Promise<void> => {
