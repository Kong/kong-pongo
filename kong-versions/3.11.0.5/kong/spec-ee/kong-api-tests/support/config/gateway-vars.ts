...
export const vars = {
  ...
  ai_providers: {
    ...
    IAM_ROLE_AWS_ACCESS_KEY_ID: process.env.IAM_ROLE_AWS_ACCESS_KEY_ID,
    IAM_ROLE_AWS_SECRET_ACCESS_KEY: process.env.IAM_ROLE_AWS_SECRET_ACCESS_KEY,
  ...
  confluent: {
    ...
    CONFLUENT_CLOUD_SR_PASSWORD: process.env.CONFLUENT_CLOUD_SR_PASSWORD, // schema registry password
  },
  solace: {
    SOLACE_CLOUD_PASSWORD: process.env.SOLACE_CLOUD_PASSWORD,
    SOLACE_CLOUD_SEMP_PASSWORD: process.env.SOLACE_CLOUD_SEMP_PASSWORD,
  }
};
/**
...
export const getIncrementalSyncStatus = async () => {
  try {
    const resp = await axios({
      url: getUrl(''),
      validateStatus: null, // Do not throw for non-2xx status
    });
    if (resp.status === 200) {
      ...
      expect.fail(`gateway admin api endpoint should return 200, got ${resp.status}`);
    }
  } catch (err) {
    expect.fail(`gateway admin api endpoint unreachable or errored: ${err}`);
  }
};