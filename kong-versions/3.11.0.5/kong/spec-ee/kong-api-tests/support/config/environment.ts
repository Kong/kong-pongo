...
export const Environment = Object.freeze({
  gateway: {
    ...
    wssProxy: 'wssProxy',
    keycloak: 'keycloak',
    keycloakMCP: 'keycloakMCP',
    keycloakMCPInternal: 'keycloakMCPInternal',
...
const getPaths = (geo = getApiGeo()) => {
  return {
    gateway: {
      ...
      keycloakMCP: `http://${getGatewayHost()}:8081`,
      // We need to use 'keycloak-mcp' as the hostname because the Kong Gateway will request
      // the keycloak-mcp container via docker network.
      keycloakMCPInternal: `http://keycloak-mcp:8081`,
