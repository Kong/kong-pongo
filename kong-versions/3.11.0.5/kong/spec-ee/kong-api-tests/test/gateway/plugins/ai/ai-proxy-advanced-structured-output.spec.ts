...
type TestConfiguration = {
  ...
  settings: {
    ...
    supportsJSONFormattedOutput?: boolean;
  };
};
...
const tests: TestConfiguration[] = [
  {
    ...
    settings: {
      ...
      supportsJSONFormattedOutput: true
    }
  },
  {
    provider: "anthropic",
    ...
    settings: {
      ...
      supportsArraySchema: false,
      supportsJSONFormattedOutput: false
    }
  },
  {
    provider: "azure",
...
const SETUP_MESSAGES: Record<string, Record<string, ChatCompletionSystemMessageParam| ChatCompletionUserMessageParam>> = {
  MATHEMATICIAN: {
    SYSTEM: {
      "role": "system",
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (attempt < maxRetries) {
        ...
        const waitTime = 4000 * Math.pow(2, attempt - 1);
        logDebug(`[RETRY] Waiting ${waitTime/1000} seconds before retry... (${maxRetries - attempt} retries left)`);
        
        await new Promise(resolve => setTimeout(resolve, waitTime));
        logDebug(`[RETRY] Starting attempt ${attempt + 1}...`);
      } else {
        console.log(`[RETRY] ${maxRetries} attempts exhausted, throwing error`);
      }
...
describe("@ai: Gateway Plugins: AI Proxy Advanced Structured Output", function () {
  ...
  for (const test of tests) {
    ...
    const makeStructuredOutputRequestObjectRoot = async (
      ...
      path: string,
      isStream = false
    ): Promise<{ data: ChatCompletion | ChatCompletionChunk[], status: number }> => {
      ...
      if (isStream) {
        ...
        return { data: events_o, status: stream.status };
      } else {
    ...
    const makeStructuredOutputRequestJSONFormattedResponse = async (
      proxyUrl: string,
      path: string,
      isStream = false
      ...
      const jsonBody: ChatCompletionCreateParamsKong = {
        "messages": [
          {
            "role": "user",
            // The last user-turn must explicitly say "JSON" for some models
            "content": "Tell me who designed the original Apple iPhone. In JSON format. Put the answer in a single JSON value with the object key \"designer\"."
          }
        ],
        "response_format": {
          "type": "json_object"
    ...
    describe(`${test.provider} (${test.model}) [text/event-stream]`, function () {
      it('structured output with object type root', async function () {
        logDebug(`Testing ${testIdentifier} at ${proxyUrl}${path} structured output with object type root`);
        await withRetry(async () => {
          const { data: events, status } = await makeStructuredOutputRequestObjectRoot(proxyUrl, path, true);

          expect(status, 'object type root stream response status should be 200').to.equal(200);
          const content = aiEventsToContent(events as ChatCompletionChunk[]);
        ...
        }, test.model === 'amazon.nova-lite-v1:0' ? 5 : 3);
      });

      it('structured output with array type root', async function () {
        logDebug(`Testing ${testIdentifier} at ${proxyUrl}${path} structured output with array type root`);
        ...
        await withRetry(async () => {
          const {data: events, status } = await makeStructuredOutputRequestArrayRoot(proxyUrl, path, true);
          expect(status, 'array type root stream response status should be 200').to.equal(200);
          const content = aiEventsToContent(events as ChatCompletionChunk[]);
      ...
      it('structured output with extreme complex schema', async function () {
        logDebug(`Testing ${testIdentifier} at ${proxyUrl}${path} structured output with extreme complex schema`);
        ...
        await withRetry(async () => {
          const { data: events, status }= await makeStructuredOutputRequestComplexSchema(proxyUrl, path, true);

          expect(status, 'extream complex schema stream response status should be 200').to.equal(200);
          const content = aiEventsToContent(events as ChatCompletionChunk[]);
          // Implicitly testing it's JSON by decode attempt
          const structuredOutput = JSON.parse(content || 'not-json');
        ...
        }, test.model === 'gemini-2.5-pro' ? 5 : test.model === 'amazon.nova-lite-v1:0' ? 5 : 3)
      });

      it('structured output with empty array', async function () {
        logDebug(`Testing ${testIdentifier} at ${proxyUrl}${path} structured output with empty array`);
        await withRetry(async () => {
          const { data: events, status} = await makeStructuredOutputRequestContainsEmptyArray(proxyUrl, path, true);
          expect(status, 'empty array type stream response status should be 200').to.equal(200);
          const content = aiEventsToContent(events as ChatCompletionChunk[]);
        ...
        }, test.model === 'amazon.nova-lite-v1:0' ? 5 : 4)
      });

      it('structured output asking for only json formatted response', async function () {
        if ('supportsJSONFormattedOutput' in test.settings && test.settings.supportsJSONFormattedOutput === false) {
          console.log("JSON formatted output is not supported for this model");
          this.skip();
        };

        await withRetry(async () => {
          const events = await makeStructuredOutputRequestJSONFormattedResponse(proxyUrl, path, true) as ChatCompletionChunk[];
    ...
    describe(`${test.provider} (${test.model}) [application/json]`, function () {
      it('structured output with object type root', async function () {
        logDebug(`Testing ${testIdentifier} at ${proxyUrl}${path} structured output with object type root`);
        await withRetry(async () => {
          const { data: res, status }  = await makeStructuredOutputRequestObjectRoot(proxyUrl, path, false);
          
          expect(status, 'object type root normal response status should be 200').to.equal(200);
          const chatCompletion = res as ChatCompletion;

          expect(chatCompletion.choices).to.have.lengthOf(1, 'Should have one message candidate in the response');
          expect((chatCompletion?.choices[0]?.message?.content || '').length, 'Should have message response textual content').to.be.gt(0);

          // Implicitly testing it's JSON by decode attempt
          const structuredOutput = JSON.parse(chatCompletion?.choices[0]?.message?.content || 'not-json');
          expect(structuredOutput).to.have.property('steps').that.is.an('array').that.is.not.empty;
          expect(structuredOutput).to.have.property('final_answer').that.is.a('string').that.is.not.empty;
        }, test.model === 'amazon.nova-lite-v1:0' ? 5 : 3)
      });
      ...
      it('structured output with array type root', async function () {
        ...
        await withRetry(async () => {
          const { data: res, status } = await makeStructuredOutputRequestArrayRoot(proxyUrl, path, false);
      ...
      it('structured output with extreme complex schema', async function () {
        ...
        await withRetry(async () => {
          const { data: res, status }  = await makeStructuredOutputRequestComplexSchema(proxyUrl, path, false);

          expect(status, 'expect complex schema normal response status should be 200').to.equal(200);
          const chatCompletion = res as ChatCompletion;
      ...
      it('structured output with empty array', async function () {
        logDebug(`Testing ${testIdentifier} at ${proxyUrl}${path} structured output with empty array`);
        await withRetry(async () => {
          const { data: res, status } = await makeStructuredOutputRequestContainsEmptyArray(proxyUrl, path, false);

          expect(status, 'empty array type normal response status should be 200').to.equal(200);
          const chatCompletion = res as ChatCompletion;
          ...
          expect(structuredOutput).to.be.an('object');
          expect(structuredOutput).to.have.property('empty_array').that.is.an('array');
        }, test.model === 'amazon.nova-lite-v1:0' ? 5 : 4) // add retry to avoid occasional 5xx failure
      });
      ...
      it('structured output asking for only json formatted response', async function () {
        ...
        await withRetry(async () => {
          const res = await makeStructuredOutputRequestJSONFormattedResponse(proxyUrl, path, false) as ChatCompletion;

          expect(res.choices).to.have.lengthOf(1, 'Should have one message candidate in the response');
          expect((res?.choices[0]?.message?.content || '').length, 'Should have message response textual content').to.be.gt(0);

          // Implicitly testing it's a JSON response by decode attempt
