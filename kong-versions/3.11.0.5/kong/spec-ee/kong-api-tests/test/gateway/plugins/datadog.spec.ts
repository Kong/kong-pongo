...
describe('Gateway Plugins: Datadog', function () {
  const datadogApiKey = vars.datadog.DATADOG_API_KEY;
  const datadogAppKey = vars.datadog.DATADOG_APPLICATION_KEY;
  ...
  before(async function () {
    ...
    await checkOrStartServiceContainer(datadogAgentContainerName, async () => {
    ...
    const unnamedRoute = await createRouteForService(serviceId, [unnamedRoutePath], {
      name: null,
      protocols: ['http'],
    });
    unNamedRouteId = unnamedRoute.id;
    const namedRoute = await createRouteForService(serviceId, [namedRoutePath], {
      name: routeName,
      protocols: ['http'],
    });
  ...
  it('should not create datadog plugin with invalid sample_rate in a metric', async function () {
    ...
    expect(resp.status, 'Status should be 400').to.equal(400);
    expect(resp.data.message, 'Should have correct error message').to.contain(
      "failed conditional validation given value of field 'stat_type'",
    );
    expect(resp.data.message, 'Should have correct error message').to.contain(
      'sample_rate = "value should be between 0 and 1"',
    );
  });
  ...
  it('should create datadog plugin with a valid payload', async function () {
    ...
    expect(resp.data.config.service_name_tag, 'Should have correct service_name_tag').to.equal(
      pluginPayload.config.service_name_tag,
    );
  ...
  it('should log events be pushed to datadog server via datadog agent', async function () {
    ...
    await eventually(
      async () => {
        ...
        if (dataDogLogs.data) {
          expect(dataDogLogs.data[0].attributes?.message, 'route path is present in event log').to.include(
            `GET ${unnamedRoutePath} HTTP/1.1`,
          );
          expect(
            dataDogLogs.data[0].attributes?.message,
            `kong_request_id ${requestId} is present in event log`,
          ).to.include(`kong_request_id: "${requestId}"`);
        }
      },
      undefined,
      5000,
  ...
  it('should push metrics events to datadog server via datadog agent', async function () {
    ...
    await eventually(
      async () => {
        ...
        expect(
          dataValues?.length ? dataValues[0][dataValues[0].length - 1] : 0,
          'metrics values should be greather than zero for the last time series',
        ).greaterThan(0);
      },
      undefined,
      5000,
  ...
  describe('metrics with consumer identifier', function () {
    ...
    const getConsumerIdentifierValue = (consumerIdentifier: string) => {
      ...
      switch (consumerIdentifier) {
        case 'custom_id':
          identifierValue = consumerCustomId;
          break;
        case 'consumer_id':
          identifierValue = consumerId.split('-').join('_');
          break;
        case 'consumer_name':
          identifierValue = consumerName;
          break;
      }
      ...
      it(`should metrics events be pushed with consumer identifier equal to ${consumerIdentifier}`, async function () {
        ...
        await eventually(
          async () => {
            ...
            expect(
              dataSeries?.length,
              'metrics series should be greather than zero representing we have consumer custom_id data points',
            ).greaterThan(0);
            if (dataSeries?.length) {
              ...
              const valuesIndex = dataSeries.findIndex(elem =>
                elem.groupTags?.includes(getConsumerIdentifierValue(consumerIdentifier)),
              );
              expect(
                dataValues?.length ? dataValues[valuesIndex][dataValues[valuesIndex].length - 1] : 0,
                'metrics values should be greather than zero for the last time series',
              ).greaterThan(0);
            }
          },
        ...
        await eventually(
          async () => {
            ...
            if (dataSeries?.length) {
              ...
              // that kong has sent increments for that metric group due to requests being sent to the proxy url
              const valuesIndex = dataSeries.findIndex(elem =>
                elem.groupTags?.includes(`route:${routeIdentifier === 'route_name' ? routeName : unNamedRouteId}`),
              );
              expect(
                dataValues?.length ? dataValues[valuesIndex][dataValues[valuesIndex].length - 1] : 0,
                'metrics values should be greather than zero for the last time series',
  ...
  after(async function () {
    await stopContainerByName('datadog');
