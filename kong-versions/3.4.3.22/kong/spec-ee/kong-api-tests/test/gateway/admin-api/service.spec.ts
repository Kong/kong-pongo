...
describe('@smoke @gke: Gateway Admin API: Services', function () {
  ...
  const assertRespDetails = (response: AxiosResponse) => {
    ...
    expect(resp.connect_timeout, 'Should have correct connect_timeout').equal(60000);
    expect(resp.read_timeout, 'Should have correct read_timeout').equal(60000);
    expect(resp.write_timeout, 'Should have correct write_timeout').equal(60000);
  ...
  it('should create a service and a route for service', async function () {
    ...
    expect(resp.data.name, 'Should have correct service name').equal(servicePayload.name);
  ...
  it('should not create a service with incorrect path', async function () {
    ...
    expect(resp.data.name, 'Should have correct error name').equal('schema violation');
    expect(resp.data.message, 'Should have correct error name').contain(`schema violation (path: should start with: /`);
  });
  ...
  it('should not get the service by wrong id', async function () {
    const resp = await getNegative(`${url}/650d4122-3928-45a1-909d-73921163bb13`, headers);
    logResponse(resp);
  ...
  it('should not delete the service when it has associated route', async function () {
    const resp = await postNegative(`${url}/${serviceId}`, {}, 'delete', headers);
    logResponse(resp);
  ...
  it('should create a service with url and path specified separately in request', async function () {
    ...
    expect(resp.data.path, 'Should have correct path').equal('/kongstrongservice');
  });
