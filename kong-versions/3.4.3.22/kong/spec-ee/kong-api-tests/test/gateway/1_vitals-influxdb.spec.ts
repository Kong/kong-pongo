...
describe('Vitals with InfluxDB Tests', function () {
  ...
  const assertKongRequestDetails = (response: any) => {
    expect(response.time, 'Should have time object in kong_request').to.not.be.null;
    ...
    expect(response.wid, 'Should have "wid" field in kong_request').to.be.string;
    expect(response.workspace, 'Should have default workspace id').to.eq(defaultWorkspaceId);

    expect(response.kong_latency, 'Should have integer kong_latency').to.be.a('number');
    expect(response.proxy_latency, 'Should have integer proxy_latency').to.be.a('number');
    expect(response.request_latency, 'Should have integer request_latency').to.be.a('number');
  };
  ...
  before(async function () {
    ...
    // start influxdb container if not already started
    await checkOrStartServiceContainer('influxdb', async () => {
      const resp = await axios.get('http://localhost:8086/ping');
      logResponse(resp);
      expect(resp.status, 'Influxdb should be live and healthy').to.equal(204);
    });
  ...
  it('should add entry in influxdb kong_request after request', async function () {
    ...
    await eventually(async () => {
      const requestEntries: any = await getAllEntriesFromKongRequest(serviceId, routeId);
      expect(requestEntries.length, 'Should have 1 entry in kong_request').to.equal(1);
      expect(requestEntries[0].kong_latency, 'Should have kong_latency').to.be.a('number');
  ...
  it('should add entry in influxdb kong_datastore_cache after request', async function () {
    ...
    expect(cacheEntries.length, 'Should have at least 1 entry in kong_datastore_cache').to.be.gte(1);
  });

  it('should see Tag keys', async function () {
    const tagKeys: any = await executeCustomQuery('show TAG keys');

    expect(tagKeys.groupRows[0].name, 'should see kong_datastore_cache tag').to.equal('kong_datastore_cache');
    expect(tagKeys.groupRows[1].name, 'should see kong_request tag').to.equal('kong_request');
    expect(tagKeys[0], 'Should see tagKey hostname ').to.haveOwnProperty('tagKey', 'hostname');
    expect(tagKeys[1], 'Should see tagKey wid').to.haveOwnProperty('tagKey', 'wid');
  });

  it('should see Field keys', async function () {
    const fieldKeys: any = await executeCustomQuery('show FIELD keys');

    expect(fieldKeys.groupRows[0].name, 'should see kong_datastore_cache field').to.equal('kong_datastore_cache');
    expect(fieldKeys.groupRows[1].name, 'should see kong_request field').to.equal('kong_request');
    expect(fieldKeys[0], 'Should see fieldKey hits').to.haveOwnProperty('fieldKey', 'hits');
    expect(fieldKeys[0], 'Should see fieldKey hits type').to.haveOwnProperty('fieldType', 'integer');
  });
  ...
  it('should have correct data in kong_datastore_cache measurement', async function () {
    ...
    expect(cacheEntries[0].time, 'Should have time object in kong_request').to.not.be.null;
    expect(cacheEntries[0].hostname, 'Should have correct hostname').to.eq(hostname);

    expect(cacheEntries[0].misses, 'Should have "misses" field').to.be.a('number');
    expect(cacheEntries[0].hits, 'Should have "hits" field').to.be.a('number');
    expect(cacheEntries[0].wid, 'Should have "wid" field in kong_datastore_cache').be.string;
  });
  ...
  it('should add multiple entries in influxdb kong_request after multiple requests', async function () {
    ...
    await eventually(async () => {
      ...
      expect(requestEntries.length, 'Should have 4 entries in kong_request').to.equal(4);
  ...
  it('should see an entry in kong_request for a request with a new route and service', async function () {
    ...
    await eventually(async () => {
      const requestEntries: any = await getAllEntriesFromKongRequest(serviceId2, routeId2);

      expect(requestEntries[0].route, 'Should have correct route id').to.eq(routeId2);
      expect(requestEntries[0].service, 'Should have correct service id').to.eq(serviceId2);
      expect(requestEntries[0].hostname, 'Should have correct hostname').to.eq(hostname);
  ...
  it('should see an entry in kong_datastore_cache for a request with a new route and service', async function () {
    ...
    expect(cacheEntries.length, 'Should have 1 entries in kong_datastore_cache').to.be.greaterThanOrEqual(1);
  });

  after(async function () {
    await clearAllKongResources();
    // await stopContainerByName('influxdb'); commenting out to avoid status-endpoint script failures
    await resetGatewayContainerEnvVariable(
      {
        KONG_VITALS: 'off',
      },
