...
import { isGwNative, isFipsMode } from 'support/config/gateway-vars';
 ...
 */
export const resetGatewayContainerEnvVariable = async (targetEnvironmentVariables: object, containerName) => {
  ...
  if (isKongNative && !isFipsMode()) {
    restartCommand = containerName === 'kong-dp1' ? `kong restart -c kong-dp.conf` : `kong restart -c kong.conf`;
  } else {
  ...
  try {
    ...
    throw new Error(`Something went wrong during updating the container environment variable: ${error}`);
  }
 ...
 */
export const startGwWithCustomEnvVars = (targetEnvironmentVariables: string) => {
  try {
    return execSync(`make envVars="${targetEnvironmentVariables}" custom_start_gw`, { stdio: 'inherit' });
  } catch (error) {
    console.log(`Something went wrong while restarting gateway with custom environment variables: ${error}`);
  }
};
 ...
 */
export const getGatewayContainerLogs = (containerName, numberOfLinesToRead = 4) => {
  const isKongNative = isGwNative();
...
export const getKongVersionFromContainer = (containerName = 'kong-cp') => {
  ...
  // If the container is not running, it will throw an error
  const containers = execSync(`docker ps --format '{{.Names}}'`).toString();
  if (!containers.includes(containerName)) {
    throw new Error(`The docker container with name ${containerName} was not found`);
  }
  // Execute the command to get the Kong version
  try {
    const version = execSync(`docker exec ${containerName} /bin/bash -c "kong version"`);
    return version.toString().trim();
  } catch (error) {
    console.log(`Something went wrong while getting kong container version: ${error}`);
  }
};
 ...
 */
export const runDockerContainerCommand = async (containerName: string, command: string) => {
...
export const deleteFileFromDockerContainer = async (containerName: string, fileName: string) => {
  ...
  try {
    ...
    console.error(`Something went wrong during deleting the file from docker container: ${error}`);
  }
};
 ...
 */
export const deployKonnectDataPlane = (
...
export const runDockerComposeCommand = command => {
  try {
    return execSync(`command="${command}" make execute_docker_compose_command`, { stdio: 'inherit' });
  } catch (error) {
...
export const updateDockerComposeFile = command => {
  try {
    return execSync(`command='${command}' make update_generated_docker_compose_file`, { stdio: 'inherit' });
  } catch (error) {
    console.log(`Something went wrong while modifying docker-compose.yml with yq command: ${error}`);
  }
};
 ...
 */
export const checkOrStartServiceContainer = async <T = void>(serviceName: string, assertions?: () => Promise<T>) => {
  try {
    // Check if the target container is running, if not, start it
    const isServiceRunning = await isContainerRunningByName(serviceName.toLocaleLowerCase());

    if (!isServiceRunning) {
      // Start container
      const command = `--profile manual up --pull missing -d ${serviceName}`;
      runDockerComposeCommand(command);

      // check container is started
      await eventually(async () => {
        const isContainerRunning = await isContainerRunningByName(serviceName.toLocaleLowerCase());
        expect(isContainerRunning, `Container ${serviceName} should be started`).to.equal(true);
      });

      // run health assertions if provided
      if (assertions) {
        await eventually(
          async () => {
            await assertions();
 ...
 * @param {string} containerName - Name of the container to check and stop
...
export const stopContainerByName = async (containerName: string) => {
  try {
    const isContainerRunning = await isContainerRunningByName(containerName.toLocaleLowerCase());
