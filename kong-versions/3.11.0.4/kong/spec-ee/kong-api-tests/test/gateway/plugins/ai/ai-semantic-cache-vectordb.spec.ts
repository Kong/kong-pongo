...
describe('@ai: Gateway Plugins: AI Semantic Cache: vectordb', function () {
  ...
  before(async function () {
    const isPgvectorRunning = await isContainerRunningByImage('pgvector/pgvector:pg17');
    if (!isPgvectorRunning) {
      runDockerComposeCommand('--profile manual up -d pgvector');
    }
  ...
  after(async function () {
    runDockerComposeCommand('rm -sf pgvector');
