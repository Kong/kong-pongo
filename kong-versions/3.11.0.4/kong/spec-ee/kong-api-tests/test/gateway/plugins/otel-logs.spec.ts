...
describe('@oss: Gateway Plugins: OpenTelemetry Logs', function () {
  ...
  before(async function () {
    const isJaegerRunning = await isContainerRunningByImage('opentelemetry-collector');
    if(!isJaegerRunning){
      runDockerComposeCommand('--profile manual up -d opentelemetry-collector');
    }
  ...
  it('should sanitize the file contents and see matching request.id in the logs with one in the traces', async function () {
    await eventually(async () => {
      await copyFileFromDockerContainer(otelColContainerName, fileName)
  ...
  after(async function () {
    runDockerComposeCommand('rm -sf opentelemetry-collector');
    await resetGatewayContainerEnvVariable(
      {
