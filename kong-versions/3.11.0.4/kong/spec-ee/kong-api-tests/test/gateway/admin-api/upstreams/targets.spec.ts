...
describe('@gke @oss: Gateway Admin API: Targets', function () {
  ...
  it('should not create duplicate of existing target using host:port', async function () {
    const resp = await postNegative(
      `${url}/${upstreamData.id}/targets`,
      { target: target },
      'post'
    ...
    expect(resp.data.message, 'should return correct error message').to.equal(
      `UNIQUE violation detected on '{target="${target}",upstream={id="${upstreamData.id}"}}'`
    );
  });
  ...
  it('should create failover target associated with upstream by upstream id', async function () {
    const resp = await axios({
      url: `${url}/${upstreamData.id}/targets`,
      method: 'post',
      data: {
    ...
    targetData.id = resp.data.id;
    targetData.target = resp.data.target;

    logResponse(resp);

    expect(resp.status, 'should return 201 status').to.equal(201);
    expect(resp.data.target, 'should return correct target').to.equal(target);
    expect(resp.data.failover, 'should return failover true').to.equal(true);
    expect(resp.data.tags, 'should return correct tags').to.contain('failover');
  });

  it('should see created failover target associated with upstream using upstream id and /all', async function () {
    const resp = await axios(`${url}/${upstreamData.id}/targets/all`);
    logResponse(resp);

    expect(resp.status, 'should return 200 status').to.equal(200);
    expect(resp.data.data.length, 'should have one target in list').to.equal(1);
    expect(resp.data.data[0].failover, 'should target be marked as failover').to.equal(true);
  });

  it('should edit failover target to be a regular target', async function () {
    const resp = await axios({
      url: `${url}/${upstreamData.id}/targets/${targetData.id}`,
      method: 'patch',
      data: {
    ...
    expect(resp.status, 'should return 200 status').to.equal(200);
    expect(resp.data.failover, 'should return failover false').to.equal(false);
  });

  it('should edit regular target to be a failover target', async function () {
    const resp = await axios({
      url: `${url}/${upstreamData.id}/targets/${targetData.id}`,
      method: 'patch',
      data: {
  ...
  it('should delete failover target', async function () {
    const resp = await axios({
      url: `${url}/${upstreamData.id}/targets/${targetData.id}`,
      method: 'delete',
    });
