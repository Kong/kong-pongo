...
local AWS_LAMBDA_SSL_CERT = pl_path.abspath("spec/fixtures/aws-lambda/server.crt")
local AWS_LAMBDA_SSL_KEY = pl_path.abspath("spec/fixtures/aws-lambda/server.key")
  ...
  http_mock = {
    lambda_plugin = [[
      ...
      server {
          ...
          listen 10000;
          listen 10001 ssl;
          ssl_certificate     ]] .. AWS_LAMBDA_SSL_CERT .. [[;
          ssl_certificate_key ]] .. AWS_LAMBDA_SSL_KEY .. [[;
              ...
              content_by_lua_block {
                local function x()
                  local function say(res, status)
                    ...
                    elseif string.match(ngx.var.uri, "functionWithProxyIntegrationEmptyArray") then
                      ngx.header["Content-Type"] = "application/json"
                      ngx.say("{\"statusCode\": 200, \"body\": \"[]\", \"headers\": {\"Content-Type\": \"application/json\"}}")

                    elseif string.match(ngx.var.uri, "functionWithLowercaseContentType") then
                      ngx.header["Content-Type"] = "application/json"
                      ngx.say("{\"statusCode\": 200, \"body\": \"test\", \"headers\": {\"content-type\": \"text/html; charset=UTF-8\"}}")

                    elseif string.match(ngx.var.uri, "functionWithContentLengthResponse") then
                      ngx.header["Content-Type"] = "application/json"
                      ngx.say("{\"statusCode\": 200, \"body\": \"{}\", \"headers\": {\"Content-Type\": \"application/json\", \"Content-Length\": 2}}")

                    elseif string.match(ngx.var.uri, "functionWithErrorForbidden") then
                      ngx.status = 403
                      local body = "{\"message\": \"The security token included in the request is invalid.\"}"
                      ngx.header["Content-Type"] = "application/json"
                      ngx.say(body)
