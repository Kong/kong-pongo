...
const createServices = async function (servicesNumber: number, workspaceName?: string) {
    console.log(`Creating ${servicesNumber} services in workspace: ${workspaceName || 'default'}`);
...
const safeCreateWorkspace = async function (name: string) {
    try {
        ...
        if (error.isAxiosError && error.response?.status === 409) {
            console.log(`Workspace ${name} already exists. Deleting and recreating...`);
...
const allTestData = JSON.parse(datafile || '{}');


for (let index = 0; index < routesFlavor.length; index++) {
    const route = routesFlavor[index];
    ...
    describe(`@smoke: Gateway Admin API: Routes priority - ${route.flavor} flavor, ${route.format.toString()} format (route_match_calculation: ${route.route_match_calculation})`, function () {
        const workspace2 = { name: 'sdet-workspace-workspace2' };
        ...
        for (let i = 0; i < testData.length; i++) {
            ...
            // Skip test case if the current route format is only traditional but the test case requires both formats
            ...
            context(testCase.scenarioName, function () {
                ...
                before(async function () {
                    try {
                        ...
                        // Still proceed - the after hook will clean up
                    }
                });
                    ...
                    try {
                        ...
                        for (const index of routeMatched) {
                            try {
                                ...
                                // Wait for incremental sync to complete if in hybrid mode
                                if (isIncSyncMode && isGwHybrid()) {
                                    await waitForConfigRebuild();
                                }
                            } catch (err) {
                                console.error(`Failed to delete route index ${index}: ${err}`);
                                // Continue with other routes even if one deletion fails
                            }
                        }
                    } finally {
                        // Ensure array is cleared even if deletion fails
                        routeMatched = [];
                        workspaceMatched = undefined;
                    }
                });
