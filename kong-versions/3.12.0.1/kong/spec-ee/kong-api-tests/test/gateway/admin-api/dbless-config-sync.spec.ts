...
const probeUrl = `${getBasePath({ environment: isGateway() ? Environment.gateway.proxy : undefined })}/probe/anything`;
const proxyUrl = `${getBasePath({ environment: isGateway() ? Environment.gateway.proxy : undefined })}/apitest/anything`;
...
(isDbless ? describe : describe.skip)('@dbless @smoke: Gateway DB-Less mode Redis Partial Config Test', function () {
    ...
    it('should query probe route path successfully', async function () {
        await eventually(async () => {
            const resp = await axios({
                method: 'get',
                url: probeUrl,
