...
describe('Gateway /licenses API tests', function () {
  ...
  */
  ...
  before(async function () {
    ...
    workspaces_count = workspaces.data.length;
  });
  ...
  it('should deploy an exipred license successfully', async function () {
    ...
    await waitForLicenseCacheStatus(serviceId); //check whether the license change took effect or not
  ...
  it('should see correct entity counts in license/report', async function () {
    await eventually(async () => {
      ...
      expect(resp.status, 'Status should be 200').to.equal(200);
      expect(resp.data.workspaces_count, 'Should see workspaces_count in response').to.eq(workspaces_count);
  ...
  it('should delete valid license successfully', async function () {
    ...
    expect(resp.status, 'Status should be 204').to.equal(204);
    await waitForLicenseCacheStatus(serviceId, false); //check whether license change took effect or not
  });
  ...
  it('should not enable key-auth-enc ee plugin without license', async function () {
    ...
    await eventually(async () => {
      ...
      ).to.include(`Enterprise license missing or expired`);
    })
  });
  ...
  it('should GET the license/report', async function () {
    ...
    // Use eventually only for the dynamic counter data that needs time to populate
    await eventually(async () => {
      resp = await axios(reportUrl);
      logResponse(resp);
      expect(resp.status, 'Status should be 200').to.equal(200);
      // Check that buckets exist and have data
      expect(resp.data.counters, 'Should have counters object').to.exist;
      expect(resp.data.counters.buckets, 'Should have buckets object').to.exist;

      const bucketKeys = Object.keys(resp.data.counters.buckets);
      expect(bucketKeys.length, 'Should have at least one bucket').to.be.greaterThan(0);

      // Validate the first bucket has request_count
      const firstBucketKey = bucketKeys[0];
      const firstBucket = resp.data.counters.buckets[firstBucketKey];

      expect(firstBucket, `Bucket ${firstBucketKey} should exist`).to.exist;
      expect(firstBucket.request_count, `Should see request_count in bucket ${firstBucketKey}`).to.exist;
      expect(firstBucket.request_count, `Request count should be >= 0`).to.be.greaterThanOrEqual(0);
    }, 60000, 2000);

    // After eventually succeeds, check the other static data using the original response
