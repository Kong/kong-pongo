...
describe('@weekly @ai: Gateway Plugins: AI Proxy Image Generation and Edit Test', function () {
  ...
  let pluginId: string;
  let skipGeminiGenerationLogsValidation = false;
  let skipGeminiEditLogsValidation = false;
   ...
   */
  async function sendImageEditRequest(url: string, prompt: string, provider?: string) {
  ...
  async function validateLogsMetrics(provider: string, model: string) {
    ...
    if (provider === 'openai' && model === 'dall-e-2') {
      // Skip token and cost assertions for OpenAI dall-e-2 as they may not be available
      logDebug(`Skipping token and cost assertions for ${provider} ${model}`);
    } else {
  ...
  function createImageTarget(provider: 'azure' | 'gemini' | 'vertex' | 'bedrock' | 'openai', operation: 'generation' | 'edit') {
    ...
    const operationConfig = {
      ...
      route_type: operation === 'generation' ? 'image/v1/images/generations' : 'image/v1/images/edits',
    };
    ...
    const providerConfigs = {
      azure: {
        auth: {
          ...
          header_value: `Bearer ${vars.ai_providers.AZUREAI_API_KEY}`,
        },
        model: {
          name: 'dall-e-3',
          options: {
  ...
  const target_azure_generation = createImageTarget('azure', 'generation');
  const target_azure_edit = createImageTarget('azure', 'edit');
  ...
  it('should create AI proxy advanced plugin with azure image generation', async function () {
    const azureImageGenPayload = _.cloneDeep(pluginPayload);
    azureImageGenPayload.service.id = serviceId;
    azureImageGenPayload.config.targets = [target_azure_generation];
    const resp = await axios({
      method: 'post',
      url: `${adminUrl}/services/${serviceId}/plugins`,
      data: azureImageGenPayload,
      validateStatus: null,
    });
  ...
  it('should be able to send prompt to azure image generation model via route and to generate image', async function () {
    const makeRequest = () => sendImageGenerationRequest(`${proxyUrl}${routePath}`, image_gen_prompt);
    await retryAIRequest(makeRequest, resp => validateImageGenerationResponse(resp, 'azure'), 'azure');
  });

  it('should be able to collect logs for azure image generation', async function () {
    await validateLogsMetrics('azure', 'dall-e-3');
  });

  it('should fail to generate image with bad prompt to azure image generation model via route', async function () {
    const resp = await sendImageGenerationRequest(`${proxyUrl}${routePath}`, '');

    logResponse(resp);
    expect(resp.status, 'Status should be 400').to.equal(400);
    expect(resp.data).to.have.property('error');
    expect(resp.data.error).to.have.property('message');
  });

  it('should be able to send prompt to azure image model via route and to edit image', async function () {
    const azureImageEditPayload = _.cloneDeep(pluginPayload);
    azureImageEditPayload.service.id = serviceId;
    azureImageEditPayload.config.targets = [target_azure_edit];

    const respData = await patchPlugin(pluginId, azureImageEditPayload);
    expect(respData.name, 'Should have correct plugin name').to.equal('ai-proxy-advanced');
    await waitForConfigRebuild();

    const makeRequest = () => sendImageEditRequest(`${proxyUrl}${routePath}/images/edits`, image_edit_prompt, 'azure');
    await retryAIRequest(makeRequest, resp => validateImageEditResponse(resp, 'azure'), 'azure');
  });

  it('should be able to collect logs for azure image edit', async () =>
    await validateLogsMetrics('azure', 'dall-e-3'));

  it('should fail to edit image with bad prompt to azure image edit model via route', async function () {
    const resp = await sendImageEditRequest(`${proxyUrl}${routePath}/images/edits`, '', 'azure');
  ...
  it('should create AI proxy advanced plugin with openai image generation', async function () {
    const openaiImageGenPayload = _.cloneDeep(pluginPayload);
    openaiImageGenPayload.service.id = serviceId;
    openaiImageGenPayload.config.targets = [target_openai_generation];
    const respData = await patchPlugin(pluginId, openaiImageGenPayload);
    pluginId = respData.id;
    expect(respData.name, 'Should have correct plugin name').to.equal('ai-proxy-advanced');
    await waitForConfigRebuild();
  });

  it('should handle bad image generations request to kong properly', async function () {
    const resp = await sendImageGenerationRequest(`${proxyUrl}${routePath}`, '');
  ...
  it('should be able to send prompt to openai image generation model via route and to generate image', async function () {
    const makeRequest = () => sendImageGenerationRequest(`${proxyUrl}${routePath}`, image_gen_prompt);
    await retryAIRequest(makeRequest, resp => validateImageGenerationResponse(resp, 'openai'), 'openai');
  });

  it('should be able to collect logs for openai image generation', async () =>
    await validateLogsMetrics('openai', 'dall-e-2'));

  it('should patch update AI proxy advanced plugin with openai image edit', async function () {
    const openaiImageEditPayload = _.cloneDeep(pluginPayload);
    openaiImageEditPayload.service.id = serviceId;
    openaiImageEditPayload.config.targets = [target_openai_edit];

    const respData = await patchPlugin(pluginId, openaiImageEditPayload);
    expect(respData.name, 'Should have correct plugin name').to.equal('ai-proxy-advanced');

    await waitForConfigRebuild();
  });

  it('should handle bad image edits request to kong properly', async function () {
    const resp = await sendImageEditRequest(`${proxyUrl}${routePath}/images/edits`, '');
    expect(resp.status, 'Status should be 400').to.equal(400);
    expect(resp.data).to.have.property('error');
    expect(resp.data.error).to.have.property('message');
  });
  ...
  it('should be able to send prompt to openai image model via route and to edit image', async function () {
    const makeRequest = () => sendImageEditRequest(`${proxyUrl}${routePath}/images/edits`, image_edit_prompt);
    await retryAIRequest(makeRequest, resp => validateImageEditResponse(resp, 'openai'), 'openai');
  });

  it('should be able to collect logs for openai image edit', async () =>
    await validateLogsMetrics('openai', 'dall-e-2'));
  ...
  it('should patch update AI proxy advanced plugin with google gemini image generation', async function () {
    ...
    const respData = await patchPlugin(pluginId, geminiImagePayload);
    expect(respData.name, 'Should have correct plugin name').to.equal('ai-proxy-advanced');

    await waitForConfigRebuild();
  });

  it('should be able to handle bad gemini image generations request to kong properly', async function () {
    const resp = await sendImageGenerationRequest(`${proxyUrl}${routePath}`, '');
  ...
  it('should be able to send prompt to gemini image generation model via route and to generate image', async function () {
    ...
    const resp = await retryAIRequest(makeRequest, resp => validateImageGenerationResponse(resp, 'gemini'), 'gemini');

    // If resp is no 200 or undefined, set the skip flag
    if (!resp || resp.status !== 200) {
      skipGeminiGenerationLogsValidation = true;
    }
  });

  it('should be able to collect logs for gemini image generation', async function () {
    if (skipGeminiGenerationLogsValidation) {
      console.warn('Skipping gemini logs validation due to not getting valid response)');
      this.skip();
    }
    await validateLogsMetrics("gemini", "gemini-2.5-flash-image");
  });
  ...
  it('should be able to handle bad gemini image edits request to kong properly', async function () {
    const resp = await sendImageEditRequest(`${proxyUrl}${routePath}/images/edits`, '');
    expect(resp.status, 'Status should be 200').to.equal(200);
  });
  ...
  it('should be able to send prompt to gemini image model via route and to edit image', async function () {
    ...
    const resp = await retryAIRequest(
      makeRequest,
      resp => {
        validateImageEditResponse(resp, 'gemini');
        return resp;
      },
      'gemini',
    );
    // If resp is no 200 or undefined, set the skip flag
    if (!resp || resp.status !== 200) {
      skipGeminiEditLogsValidation = true;
    }
  });

  it('should be able to collect logs for gemini image edit', async function () {
    if (skipGeminiEditLogsValidation) {
      console.warn('Skipping gemini logs validation due to not getting valid response)');
      this.skip();
    }
    await validateLogsMetrics("gemini", "gemini-2.5-flash-image")
  ...
  it('should patch update AI proxy advanced plugin config support google vertex image generation', async function () {
    ...
    const respData = await patchPlugin(pluginId, vertexImagePayload);
    expect(respData.name, 'Should have correct plugin name').to.equal('ai-proxy-advanced');

    await waitForConfigRebuild();
  });

  it('should be able to handle bad vertex image generations request to kong properly', async function () {
    const resp = await sendImageGenerationRequest(`${proxyUrl}${routePath}`, '');
    // Vertex seems to return 200 with empty data for bad requests
    expect(resp.status, 'Status should be 200').to.equal(200);
  });
  ...
  it('should be able to send prompt to vertex image generation model via route and to generate image', async function () {
    ...
    await retryAIRequest(makeRequest, resp => validateImageGenerationResponse(resp, 'vertex'), 'vertex');
  });

  it('should be able to collect logs for vertex image generation', async () => (await validateLogsMetrics("gemini", "gemini-2.5-flash-image")));

  it('should patch update AI proxy advanced plugin with google vertex image edit', async function () {
    const vertexImageEditPayload = _.cloneDeep(pluginPayload);
    vertexImageEditPayload.service.id = serviceId;
    vertexImageEditPayload.config.targets = [target_vertex_edit];

    const respData = await patchPlugin(pluginId, vertexImageEditPayload);
    expect(respData.name, 'Should have correct plugin name').to.equal('ai-proxy-advanced');
  ...
  it('should be able to send prompt to vertex image model via route and to edit image', async function () {
    ...
    await retryAIRequest(makeRequest, resp => validateImageEditResponse(resp, 'vertex'), 'vertex');
  });

  it('should be able to collect logs for vertex image edit', async () => (await validateLogsMetrics("gemini", "gemini-2.5-flash-image")));

  it('should patch update AI proxy advanced plugin config support aws bedrock image generation', async function () {
    const bedrockImageGenPayload = _.cloneDeep(pluginPayload);
    bedrockImageGenPayload.service.id = serviceId;
    bedrockImageGenPayload.config.targets = [target_bedrock_generation];

    const respData = await patchPlugin(pluginId, bedrockImageGenPayload);
    expect(respData.name, 'Should have correct plugin name').to.equal('ai-proxy-advanced');

    await waitForConfigRebuild();
  });

  it('should be able to handle bad bedrock image generations request to kong properly', async function () {
    const resp = await sendImageGenerationRequest(`${proxyUrl}${routePath}`, '');
    expect(resp.status, 'Status should be 400').to.equal(400);
  });
  ...
  it('should be able to send prompt to bedrock image generation model via route and to generate image', async function () {
    ...
    await retryAIRequest(makeRequest, resp => validateImageGenerationResponse(resp, 'bedrock'), 'bedrock');
  });

  it('should patch update AI proxy advanced plugin with aws bedrock image edit', async function () {
    const bedrockImageEditPayload = _.cloneDeep(pluginPayload);
    bedrockImageEditPayload.service.id = serviceId;
    bedrockImageEditPayload.config.targets = [target_bedrock_edit];

    const respData = await patchPlugin(pluginId, bedrockImageEditPayload);
    expect(respData.name, 'Should have correct plugin name').to.equal('ai-proxy-advanced');
    await waitForConfigRebuild();
  });

  it('should be able to handle bad bedrock image edits request to kong properly', async function () {
    const resp = await sendImageEditRequest(`${proxyUrl}${routePath}/images/edits`, '');
    expect(resp.status, 'Status should be 400').to.equal(400);
  });
  ...
  it('should be able to send prompt to bedrock image model via route and to edit image', async function () {
    ...
    await retryAIRequest(makeRequest, resp => validateImageEditResponse(resp, 'bedrock'), 'bedrock');
  });
