...
describeWithWorkspaces('@smoke: Gateway Plugins: mtls-auth', function () {
  this.timeout(40000)
  ...
  it('should patch mtls-auth plugin anonymous consumer', async function () {
    const resp = await axios({
      ...
      data: {
        config: {
          anonymous: consumer3Details.username,
    ...
    expect(resp.status, 'Status should be 200').to.equal(200);
    expect(resp.data.config.consumer_by, 'Should see correct consumer_by configuration').to.eql(['custom_id']);
    expect(resp.data.config.skip_consumer_lookup, 'Should see skip_consumer_lookup disabled').to.be.false
    expect(resp.data.config.anonymous, 'Should see correct anonymous configuration').to.equal(consumer3Details.username)
  });

  it('should fallback to the given anonymous consumer when authentication fails', async function () {
    // should return errors when anonymous consumer doesn't exist
    await eventually(async () => {
      ...
      expect(resp1.data.message, 'Should see correct error message').to.equal(`anonymous consumer consumer3 is configured but doesn't exist`)
    })

    // should create an anonymous consumer
    const consumer3 = await createConsumer(consumer3Details.username, consumer3Details);
    consumer3Details.id = consumer3.id

    // should fall back to the given anonymous consumer on auth failure
    await eventually(async () => {
      ...
      expect(resp2.data.headers['X-Consumer-Id'], 'Should see X-Consumer-Id header').to.equal(consumer3Details.id);
      expect(resp2.data.headers['X-Consumer-Username'], 'Should see X-Consumer-Username header').to.equal(consumer3Details.username);
      expect(resp2.data.headers['X-Anonymous-Consumer'], 'Should see X-Anonymous-Consumer header').to.equal('true')
    })
  });
