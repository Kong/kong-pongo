...
describeWithWorkspaces('@gke: Gateway Plugins: key-auth-enc', function () {
  ...
  const tagAndTtlPayload = { tags: ['tag1'], ttl: keyTtl, key: keyId };
  ...
  //This test cover https://konghq.atlassian.net/browse/FTI-7021, need to run in incremental sync mode and non-default ws
  it('key-auth-enc: should not proxy request before consumer credential added with valid apiKey', async function () {
