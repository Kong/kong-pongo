...
import {
  ...
  deleteAILogCollectingRoute
} from '@support'
import { GoogleGenAI, GenerateContentResponse } from '@google/genai';
import { BedrockRuntimeClient, InvokeModelCommand, InvokeModelWithResponseStreamCommand,
        ConverseCommand, ConverseStreamCommand, ConversationRole, ConverseCommandOutput,
...
import { Message, TextBlock, TextDelta } from '@anthropic-ai/sdk/resources/messages';
type RankResponse = protos.google.cloud.discoveryengine.v1.IRankResponse;


describe('@ai: Gateway Plugins: AI Proxy native format test', function () {
  ...
  const providers: ProviderConfig[] = [
    ...
    // test with converseCommand
    {
      name: 'bedrock',
      case: 'bedrock-chat-converse',
      model: 'amazon.nova-lite-v1:0', // do not use micro, it doesn't support on-demand inference
      options: {
        "bedrock": {
    ...
    // TODO: not working, bedrock returns 200 response with empty output
    // {
    //   name: 'bedrock',
    //   case: 'bedrock-chat-converse-arn',
    //   //model: `arn:aws:bedrock:${aws_region}::inference-profile/anthropic.claude-3-haiku-20240307-v1:0`,
    //   model: `arn:aws:bedrock:${aws_region}:267914366688:inference-profile/us.anthropic.claude-3-haiku-20240307-v1:0`,
    //   options: {
    //     "bedrock": {
    //       "aws_region": aws_region
    //     }
    ...
    {
      name: 'bedrock',
      case: 'bedrock-chat-invoke',
      // model: `arn:aws:bedrock:${aws_region}::foundation-model/anthropic.claude-3-haiku-20240307-v1:0`,
      // titan doesn't support InvokeModelWithResponseStream?
  ...
  function parseNativeResponse(resp: AxiosResponse<any, any>| RankResponse| GenerateContentResponse| RerankCommandOutput| ConverseCommandOutput| object, providerCase: string): string {
    ...
    if (providerCase === 'gemini-chat' || providerCase === 'gemini-vertex-chat') {
    ...
    } else if (providerCase.startsWith('bedrock-chat-converse')) {
      const r = resp as ConverseCommandOutput;
      expect(r.output, 'Output should be an object').to.be.an('object');
      expect(r.output?.message?.content, 'First message content should be an array').to.be.an('array');
      expect(r.output?.message?.content?.[0]?.text, 'First message text should be a string').to.be.a('string');
      return r.output?.message?.content?.[0]?.text ?? '';
    } else if (providerCase.startsWith('bedrock-chat-invoke')) {
      const r = resp as any; // Bedrock invoke command chat doesn't have type, awesome!
      expect(r.usage, 'usage should be an object').to.be.an('object');
      expect(r.usage.output_tokens, 'Output tokens should be greater than 1').to.be.greaterThan(1);
      expect(r.usage.input_tokens, 'Input tokens should be greater than 1').to.be.greaterThan(1);
      return r.output?.content?.[0]?.text ?? '';
      ...
      return '';
    } else if (providerCase === 'anthropic-chat') {
      const r = resp as Message;
      expect(r.usage, 'usage should be an object').to.be.an('object');
      expect(r.usage.output_tokens, 'Output tokens should be greater than 1').to.be.greaterThan(1);
      expect(r.usage.input_tokens, 'Input tokens should be greater than 1').to.be.greaterThan(1);
  ...
  async function validateLogsMetrics(provider: ProviderConfig, requestMode: string): Promise<void> {
    if (provider.case.endsWith('-rerank')) {
      return; // Rerank does not generate metrics in the same way as chat models
    }

    const logsResp = await axios({
      method: 'get',
      url: `${proxyUrl}${logsCollectingPathPrefix}/${provider.case}`,
      validateStatus: null
    });
    ...
    if (provider.case.search('-chat') != -1) {
      expect(logs, 'Logs should have proxy property').to.have.property('proxy');
      expect(logs.proxy, 'Proxy should have tried_targets property').to.have.property('tried_targets');
      expect(logs.proxy, 'Proxy should have meta property').to.have.property('meta');
      expect(logs.proxy.meta, 'Meta should have llm_latency').to.have.property('llm_latency').greaterThan(0);
      expect(logs.proxy.meta, 'Meta should have provider_name').to.have.property('provider_name').equal(provider.name);
      expect(logs.proxy.meta, 'Meta should have request_mode').to.have.property('request_mode').equal(requestMode);
      expect(logs.proxy.meta, 'Meta should have request_model').to.have.property('request_model').equal(provider.model);
      expect(logs.proxy.meta, 'Meta should have response_model').to.have.property('response_model').equal(provider.model);
      expect(logs.proxy.usage, 'Usage should have time_per_token').to.have.property('time_per_token').greaterThan(0);
      expect(logs.proxy.usage, 'Usage should have time_to_first_token').to.have.property('time_to_first_token').greaterThan(0);
      expect(logs.proxy.usage, 'Usage should have completion_tokens_details').to.have.property('completion_tokens_details');
      expect(logs.proxy.usage, 'Usage should have completion_tokens').to.have.property('completion_tokens').greaterThan(0);
      expect(logs.proxy.usage, 'Usage should have prompt_tokens_details').to.have.property('prompt_tokens_details');
      expect(logs.proxy.usage, 'Usage should have prompt_tokens').to.have.property('prompt_tokens').greaterThan(0);
      expect(logs.proxy.usage, 'Usage should have total_tokens').to.have.property('total_tokens').greaterThan(0);
    } else {
      expect.fail(`Logs validation for ${provider.case} is not implemented`);
    }
  }
  ...
  before(async function () {
    ...
    // It seems this will cause tests to stable, not sure why though
    await clearAllKongResources()

    //create a service and route for use with plugin
    const service = await createGatewayService('ai-test-service')
    serviceId = service.id
    const route = await createRouteForService(serviceId, [path, pathGcp])
    routeId = route.id
  ...
  providers.forEach((provider) => {
    ...
    it(`should create AI proxy plugin using ${provider.case} provider and model ${provider.model} model`, async function () {
      ...
      await createAILogCollectingRoute(`ai-log-service-${provider.case}`, routeId, `${logsCollectingPathPrefix}/${provider.case}`);
    ...
    it(`should send ${provider.case} native format API request and verify response`, async function () {
      let responseText = '';
      if (provider.case === 'gemini-chat' || provider.case === 'gemini-vertex-chat') {
        const client = provider.case === 'gemini-chat' ? geminiAI : vertexAI;
        try {
      ...
      } else if (provider.case.startsWith('bedrock-chat-converse')) {
        try { 
          const payload = {
          ...
          const response = await bedrockClient.send(new ConverseCommand(payload));

          responseText = parseNativeResponse(response, provider.case);
        } catch (err) {
          handleNormalError(err, provider.case);
        }
      } else if (provider.case.startsWith('bedrock-chat-invoke')) {
        try {
          const payload = {
      ...
      } else if (provider.case === 'anthropic-chat') {
        const resp = await anthropicClient.messages.create({
          max_tokens: 1024,
          messages: [{ role: 'user', content: promptText }],
          model: provider.model,
        });
        responseText = parseNativeResponse(resp, provider.case);
      }
    ...
    it(`should successfully generate metrics for ${provider.case} with logs`, async () => { await validateLogsMetrics(provider, "oneshot") })
    ...
    it(`should be able to send message to ${provider.case} AI model ${provider.case} via route with streaming enabled`, async function () {
      if (provider.case.endsWith('-rerank')) { // Skip streaming test beceause rerank does not support streaming
        logDebug('Skipping streaming test for rerank model');
        this.skip();
      }
      ...
      if (provider.case === 'gemini-chat' || provider.case === 'gemini-vertex-chat') {
        ...
        try { 
          ...
          const response = await bedrockClient.send(new ConverseStreamCommand(payload));
        ...
        try {
          ...
          if (!completeMessage.trim()) {
            ...
            logDebug('Parsed response body: ' + responseText);
          }
        } catch (err) {
          handleNormalError(err, provider.case);
        }
    ...
    it(`should successfully generate metrics when streaming for ${provider.case} with logs`, async function () {
      if (provider.case.endsWith('-rerank') || provider.case.startsWith('bedrock-chat-invoke')) { 
        // Skip streaming test beceause rerank does not support streaming
        // Skip invoke stream analytics because we don't support metrics of that mode
        this.skip();
      }

      await validateLogsMetrics(provider, "stream")
    })
    ...
    it(`should delete AI proxy plugin for ${provider.case}`, async function () {
      ...
      await deleteAILogCollectingRoute(`ai-log-service-${provider.case}`);
