...
// This test verify the responses, files, batches, assistants functionality of the AI Proxy advanced plugin with Openai llm format.
describe('@ai: Gateway Plugins: AI Proxy Advanced Restful Endpoints Test', function () {
  ...
  type restfulEntitiesProvider = 'openai' | 'azure' | 'mockserver';
  const providers: restfulEntitiesProvider[] = ['openai', 'azure', 'mockserver'];
  ...
  function generateFilesPayload(provider: restfulEntitiesProvider) {
  ...
  function generateBatchesPayload() {
    return {
      data: {
  ...
  function generateAssistantsPayload() {
    return {
      data: {
        name: 'some name',
      },
  ...
  const ENTITY_TEST_FLOWS: Record<restfulEntity, EntityConfig> = {
    responses: {
      ...
      payloadGenerator: generateResponsesPayload,
    },
    files: {
      ...
      payloadGenerator: generateFilesPayload,
    },
    batches: {
      ...
      payloadGenerator: generateBatchesPayload,
    },
    assistants: {
      operations: ['create', 'get', 'list', 'delete'],
      specialOperations: [],
      ...
      payloadGenerator: generateAssistantsPayload,
    },
  };
  ...
  function createRestfulTarget(provider: restfulEntitiesProvider, entity: restfulEntity) {
    ...
    const providerConfigs = {
      ...
      mockserver: {
        auth: {
          header_name: 'api-key',
          header_value: `mocked_api_key`,
        },
        model: {
          name: targetModels.mockserver,
          options: {
            upstream_url: 'http://httpbin/status/400',
            input_cost: 1,
            output_cost: 1,
          },
          provider: 'openai',
  ...
  async function createRestfulEntity(
    url: string,
    entityType: restfulEntity,
    provider: restfulEntitiesProvider,
    ...
    console.log('Creating entity at URL:', url, 'with payload:', payload);
    ...
    switch (operationType) {
      ...
      case 'list':
        ...
        if (data.usage) {
          ...
          logDebug(
            `Token Usage - Input: ${data.usage.input_tokens || 'N/A'}, ` +
  ...
  it('should create AI proxy advanced plugin with all targets for restful entities test', async function () {
    providers.forEach(async provider => {
      entities.forEach(async entity => {
        ...
        const targetPerProvider = createRestfulTarget(provider, entity);

        targetPayload.config.targets = [targetPerProvider];
        // Only set model_name_header for 'responses' entity
        targetPayload.config.model_name_header = entity === 'responses';
        targetPayload.service = { id: serviceId };
        targetPayload.route = { id: route.id };

        const respData = await createPlugin(targetPayload);
  ...
  providers.forEach(provider => {
    entities.forEach(entity => {
      ...
      if (provider === 'mockserver') {
        if (ENTITY_TEST_FLOWS[entity].operations.includes('create')) {
          it(`should fail to create ${entity} from ${provider} via ai-proxy-advanced plugin`, async function () {
            const resp = await createRestfulEntity(`${proxyUrl}${routePath}`, entity, provider);
      ...
      if (ENTITY_TEST_FLOWS[entity].operations.includes('create')) {
        it(`should successfully create ${entity} from ${provider} via ai-proxy-advanced plugin`, async function () {
          ...
          if (entity === 'batches') {
            const createFile = () => createRestfulEntity(`${proxyUrl}/${provider}/files`, 'files', provider);
            const fileValidator = ENTITY_TEST_FLOWS['files'].validator;
            const fileData = await retryAIRequest(
              createFile,
              resp => fileValidator(resp, provider, 'create'),
              provider,
            );
            expect(fileData, 'Files response data should have id property').to.have.property('id');
            dynamicFields['input_file_id'] = fileData.id;
          }

          const createEntity = () => createRestfulEntity(`${proxyUrl}${routePath}`, entity, provider, dynamicFields);
          const entityData = await retryAIRequest(createEntity, resp => validate(resp, provider, 'create'), provider);
      ...
      if (ENTITY_TEST_FLOWS[entity].operations.includes('get')) {
        it(`should successfully get created ${entity} from ${provider} via ai-proxy-advanced plugin`, async function () {
          expect(entityId, `Entity ID for ${entity} should be set from previous test`).to.exist.and.to.not.equal('');
          const fetchEntity = () => getRestfulEntity(`${proxyUrl}${routePath}`, entityId);
          await retryAIRequest(fetchEntity, resp => validate(resp, provider, 'get'), provider);
        });
      }
      ...
      if (ENTITY_TEST_FLOWS[entity].operations.includes('list')) {
        it(`should successfully list ${entity} from ${provider} via ai-proxy-advanced plugin`, async function () {
          const makeRequest = () => listRestfulEntity(`${proxyUrl}${routePath}`);
          await retryAIRequest(makeRequest, resp => validate(resp, provider, 'list'), provider);
        });
      }
      ...
      if (ENTITY_TEST_FLOWS[entity].specialOperations.includes('inputItems')) {
        it(`should successfully get input items from created ${entity} from ${provider} via ai-proxy-advanced plugin`, async function () {
          expect(entityId, `Entity ID for ${entity} should be set from previous test`).to.exist.and.to.not.equal('');
          const fetchInputItems = () => getResponseInputItem(`${proxyUrl}${routePath}`, entityId, provider);
          await retryAIRequest(fetchInputItems, resp => validate(resp, provider, 'inputItems'), provider);
        });
      }
      ...
      if (ENTITY_TEST_FLOWS[entity].specialOperations.includes('content')) {
        it(`should successfully get content from created ${entity} from ${provider} via ai-proxy-advanced plugin`, async function () {
          ...
          const makeRequest = () => getRestfulEntity(`${proxyUrl}${routePath}`, `${entityId}/content`);
          await retryAIRequest(
            makeRequest,
            // content returns non JSON
            resp => {
              expect(resp, 'Response should have data property').to.have.property('data');
              expect(resp.data, 'Data to contain the file content we uploaded').to.include(
      ...
      if (ENTITY_TEST_FLOWS[entity].operations.includes('delete')) {
        it(`should successfully delete created ${entity} from ${provider} via ai-proxy-advanced plugin`, async function () {
          expect(entityId, 'Entity ${entity} ID should be available from previous test').to.exist;
          expect(entityId, 'Entity ${entity} ID should not be null').to.not.equal('');
          const makeRequest = () => deleteRestfulEntity(`${proxyUrl}${routePath}`, entityId);

          await retryAIRequest(makeRequest, resp => validate(resp, provider, 'delete'), provider);
        });
      }
    });
  });
