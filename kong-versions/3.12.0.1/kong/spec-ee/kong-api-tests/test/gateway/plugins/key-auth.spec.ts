...
describeWithWorkspaces('@smoke @koko @oss: Gateway Plugins: key-auth', function () {
  const path = '/key-auth';
  ...
  const keyTtl = isGwHybrid() && !isLocalDatabase() ? 40 : 18;
  ...
  it('should enable key-auth plugin with apiKey in header', async function () {
    ...
    expect(resp.config.key_in_header, 'Default value is True').to.be.true;
    expect(resp.config.key_in_body, 'Default value is False').to.be.false;
    expect(resp.config.key_in_query, 'Default value is True').to.be.true;
    expect(resp.enabled, 'Should be true').to.be.true;
    expect(resp.config.key_names[0], 'Header key is apiKey').to.contain(
      key
    );
  ...
  it('should proxy request when anonymous consumer is set', async function () {
    const consumerName = 'tester';
    const pluginUpdateMethod =  isKoko() ? 'put' : 'patch'; // *** KOKO DOES NOT PATCH PLUGIN ***
    const queryUrl = `${proxyUrl}${path}?api_key=invalidkey`;

    const payload1 = {
      name: 'key-auth',
      config: {
        anonymous: consumerName,
      },
    };
    const resp1 = await updatePlugin(pluginId, payload1, pluginUpdateMethod);  
    logResponse(resp1);
    expect(resp1.status, 'Status should be 200').to.equal(200);
    expect(resp1.data.config.anonymous, `Anonymous should be tester`).to.equal(consumerName);

    // request with API key should fail because no anonymous consumer exists
    await eventually(async () => {
      const resp = await axios({ method: 'get', url: queryUrl, validateStatus: null });
      logResponse(resp);
      ...
      expect(resp.data.message, 'Should return correct error message').to.equal(
        `anonymous consumer tester is configured but doesn't exist`
      );
    });
    ...
    // request with API key should now succeed
    await eventually(async () => {
      const resp = await axios({ method: 'get', url: queryUrl, validateStatus: null });
      logResponse(resp);
      expect(resp.status, 'Status should be 200').to.equal(200);
    ...
    // reset plugin to remove anonymous consumer
    const payload2 = {
      name: 'key-auth',
      config: {
        anonymous: null,
    ...
    const resp2 = await updatePlugin(pluginId, payload2, pluginUpdateMethod);  
    expect(resp2.status, 'Status should be 200').to.equal(200);
    expect(resp2.data.config.anonymous, `Anonymous should be null`).to.equal(null);
  })
