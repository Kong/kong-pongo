...
describe('@ai: Gateway Plugins: AI Proxy', function () {
  this.timeout(360000); // 360 seconds for all tests in this suite
  //providers do not support preserve will skip route_type 'preserve' tests
  const excludedCompletionVariants = new Set(['gemini', 'vertex', 'azure', 'bedrock', 'huggingface']);
  const providers = [
    {
      name: 'openai',
      variant: 'openai',
    ...
    //huggingface via API varies based on model and requires a custom upstream_url
    {
      name: 'huggingface',
      variant: 'huggingface',
      chat: {
      ...
      auth_header: 'Authorization',
      auth_key: `Bearer ${vars.ai_providers.HUGGINGFACE_API_KEY}`
    },
  ]
  ...
  providers.forEach((provider) => {
    ...
    it(`should see statistics in logs when log_statistics is enabled for ${provider.variant} AI proxy plugin`, async function () {
      ...
      await copyFileFromDockerContainer(kongContainerName, logPath);
      ...
      const regex_p = new RegExp(`"prompt_tokens":(\\ ?)[1-9].*?(,|})`)
      expect(logContent, 'should include prompt token usage').to.match(regex_p)
      const regex_c = new RegExp(`"completion_tokens":(\\ ?)[1-9].*?(,|})`)
      expect(logContent, 'should include completion token usage').to.match(regex_c)
    })

    it(`should be able to disable logging statistics for ${provider.variant} AI proxy plugin`, async function () {
      const resp = await axios({
        method: 'patch',
        url: `${adminUrl}/services/${serviceId}/plugins/${pluginId}`,
        data: {
    ...
    if (provider.completions.model) {
      ...
      it(`should see statistics in logs when log_statistics is enabled for ${provider.variant} AI proxy plugin`, async function () {
        ...
        await eventually(async () => {
          ...
          expect(logContent, 'should contain usage statistics').to.contain('usage')
          const regex_p = new RegExp(`"prompt_tokens":\\d`)
          expect(logContent, 'should include prompt token usage').to.match(regex_p)
          const regex_c = new RegExp(`"completion_tokens":\\d`)
          expect(logContent, 'should include completion token usage').to.match(regex_c)
        }, 50000);
      })
      ...
      it(`should be able to revert route type from completions to chat route for ${provider.variant} AI proxy plugin`, async function () {
        pluginPayload.config.model.name = provider.chat.model as string
        pluginPayload.config.model.options = provider.chat.options || {}

        const updatedPluginPayload = {
          config: {
            route_type: 'llm/v1/chat',
            model: {
              name: pluginPayload.config.model.name,
        ...
        expect(respData.config.route_type, 'Should have correct route type').to.equal('llm/v1/chat')
    ...
    it(`should be able to stream responses per request when stream is set to true for ${provider.variant} AI proxy plugin`, async function () {
      ...
      logResponse(resp)
      expect(resp.status, 'Should have 200 status code').to.equal(200)
      expect(resp.headers['transfer-encoding'], 'should have Transfer-Encoding header set to chunked').to.equal('chunked')
      ...
      expect(logContent, 'Should not contain payload').to.not.contain(`{"messages":"`)
      expect(logContent, 'should contain usage statistics').to.contain('usage')
      const regex_p = new RegExp(`"prompt_tokens":(\\ ?)[1-9].*?(,|})`)
      expect(logContent, 'should include prompt token usage').to.match(regex_p)
      const regex_c = new RegExp(`"completion_tokens":(\\ ?)[1-9].*?(,|})`)
