...
describe('@ai: Gateway Plugins: AI Proxy Advanced Embedding Test', function () {
  ...
  const emb_models: { [p in EmbeddingProvider]: string } = {
    ...
    "gemini-vertex": "gemini-embedding-001", // use this model which is supported for token counting
  ...
  before(async function () {
    // TODO: Add token counting check for other providers
    ...
    const res = await gemini.models.countTokens({
      ...
      model: emb_models['gemini-vertex'],
      contents: [
        {
    ...
    logDebug(`Gemini Vertex token count response: ${res.totalTokens}}`);
    if (res.totalTokens) {
      // We test batch request with 2 identical inputs for gemini-vertex
      geminiVertexTokenCount = res.totalTokens * 2;
    }
  ...
  providers.forEach((provider) => {
    ...
    it(`should fail to retrieve embeddings from ${provider} with invalid input`, async function () {
      // verify the reqest fails when invalid encoding format is sent
      const url = `${proxyUrl}${providerTestData[provider].routePath}/embeddings`;
      
      const response = await sendEmbeddingsRequest(url, null as any);
      logResponse(response);
      expect(response.status, `Response status should be 400 for ${provider}`).to.equal(400);
      expect(response.data.error.message, `Error message should indicate missing input for ${provider}`).to.include('request body doesn\'t contain valid inputs');
    });
    ...
    it(`should successfully retrieve embeddings from ${provider} via ai-proxy-advanced plugin with logs`, async function () {
      ...
      if (provider !== 'gemini' && provider !== 'huggingface') {
        if (provider === 'gemini-vertex') {
          expect(logs.proxy.usage.prompt_tokens, `Prompt tokens should be present for ${provider}`).to.be.equal(geminiVertexTokenCount);
          expect(logs.proxy.usage.total_tokens, `Total tokens should be present for ${provider}`).to.be.equal(geminiVertexTokenCount);
      ...
      expect(logs.proxy.tried_targets, `Tried targets should be present for ${provider}`).to.exist;
    })
