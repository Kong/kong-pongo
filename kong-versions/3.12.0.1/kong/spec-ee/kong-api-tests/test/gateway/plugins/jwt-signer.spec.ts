...
describeWithWorkspaces('Gateway Plugins: jwt-signer', function () {
  this.timeout(50000);
  const path = '/jwtsigner';
  const serviceName = 'jwt-signer-service';
  const consumerName = 'demo';
  const consumerNameInitial = 'damie';
  ...
  const validTokenHeadersInitial = {
    Authorization: `bearer ${authDetails.valid_tokenInitial}`,
  };
  ...
  const testClaims = [
    ...
    {
      description:
        'should not be able to remove claims if added with add_claims or add_access_token_claims in the resigned token',
      config: {
    ...
    },
    {
      ...
      expectedToken: { iat: 1665610129, exp: 9999999999, iss: 'demo', username: 'demo' },
      headerKey: 'Og-Token',
      tokenHeaders: validTokenHeaders,
    },
    {
  ...
  before(async function () {
    ...
    routeId = route.id;
    const consumer = await createConsumer(consumerNameInitial);
  ...
  it('should create jwt-signer plugin with default parameters when config payload is not supplied by the user', async function () {
    ...
    expect(resp.data.config.verify_access_token_expiry, 'Expiry should be true').to.true;
    expect(resp.data.config.access_token_issuer, 'Issuer should be kong').to.equal('kong');
    expect(resp.data.config.access_token_signing_algorithm, 'Algorithm should be RS256').to.equal('RS256');
    expect(resp.data.config.access_token_jwks_uri, 'Jwks uri should be null').to.be.null;
  ...
  it('should patch jwt-signer plugin to set JWKS_URI allowing token validation', async function () {
    ...
    expect(resp.data.config.access_token_jwks_uri, 'Jwks uri should not be null').to.equal(jwksUri);

    await waitForConfigRebuild();
  });

  it('should proxy request with a valid token with initial consumer name', async function () {
    const req = () =>
      axios({
        headers: validTokenHeadersInitial,
        url: `${proxyUrl}${path}`,
      });
    ...
    const assertions = resp => {
      ...
      ).to.equal(consumerNameInitial);
      expect(resp.data.headers[upstreamConsumerHeaderId], 'Should see consumer id in upstream request').to.equal(
        consumerId,
      );
    };
  ...
  it('should not proxy request with a expired token ', async function () {
    ...
    expect(resp.status, 'Status should be 401').to.equal(401);
    expect(resp.data.message, 'Should be Unauthorized').to.equal('Unauthorized');
  });

  it('should change consumer name successfully', async function () {
    const resp = await patchConsumer(consumerId, { username: consumerName });
    expect(resp.username, 'Username should be updated').to.equal(consumerName);

    await waitForConfigRebuild();
  });

  it('should not proxy request with a valid token with initial consumer name', async function () {
    const req = () => getNegative(`${proxyUrl}${path}`, validTokenHeadersInitial);
    ...
    try {
      ...
      // Add context about which test failed
      if (error instanceof Error) {
        error.message = `Test "${this.test?.title}" failed: ${error.message}`;
        throw error;
      } else {
        throw new Error(`Test "${this.test?.title}" failed: ${String(error)}`);
      }
    }
  });

  it('should proxy request with a valid token with updated consumer name', async function () {
    const req = () =>
      axios({
