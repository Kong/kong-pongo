...
describeWithWorkspaces('Gateway Plugins: OIDC with Keycloak', function () {
    ...
    const expiredCertificate = authDetails.keycloak.expired_cert
    const expiredKey = authDetails.keycloak.expired_key
    ...
    before(async function () {
        ...
        // add valid certificates to Kong
        ...
        // add invalid certificates to Kong
        ...
        // add expired certificates to Kong
        const expiredCert = await axios({
            method: 'POST',
            url: `${urls.admin}/certificates`,
            data: {
                cert: expiredCertificate,
                key: expiredKey,
            },
            validateStatus: null,
        })
        expiredCertId = expiredCert.data.id
    ...
    it('should be rejected with cert + token if consumer_groups_claim is invalid', async function () {
        // update plugin to use invalid consumer_groups_claim        
        const resp = await axios({
            method: 'PATCH',
            url: `${urls.admin}/plugins/${oidcPluginId}`,
            data: {
                config: {
                    consumer_groups_claim: ["kong-abc"]
                },
            },
            validateStatus: null,
        })
        ...
        // shoud be rejected as no valid consumer group is found for the claim value kong-abc
        await eventually(async () => {
            const resp = await axios({
                method: 'GET',
                url: `${urls.proxySec}${keycloakPath}`,
            ...
            expect(resp.data.message, 'Message should be "Forbidden"').to.equal('Forbidden');
        })
    })

    it('should succeed with cert + token if consumer_groups_claim is valid', async function () {
        // update plugin to use valid consumer_groups_claim
        const payload = {
            config: {
                consumer_groups_claim: ["kong-group"] // valid claim in Keycloak token that contains the consumer groups(group-1)
            }
        }
        await patchPlugin(oidcPluginId, payload)

        // request should fail as no valid consumer is created in kong for the claim value kong-abc
        await eventually(async () => {
            const resp = await axios({
                method: 'GET',
                url: `${urls.proxySec}${keycloakPath}`,
        ...
        // create consumer with matching consumer group
        await createConsumerGroup(consumerGroupName)

        // should succeed with cert + token if consumer_groups_claim is valid
        await eventually(async () => {
            const resp = await axios({
                method: 'GET',
                url: `${urls.proxySec}${keycloakPath}`,
    ...
    it('should be rejected when the consumer group name is updated to an invalid value', async function () {
        // update the name of consumer group to an invalid value     
        await patchConsumerGroup(consumerGroupName, { name: 'update-group-name-test' })
    ...
    it('should succeed with cert + token if consumer_claim is valid', async function () {
        // update plugin to use valid consumer_claim
        const payload = {
            config: {
                consumer_groups_claim: [], // reset consumer_groups_claim to empty array
        ...
        // create consumer with matching consumer_claim value
        await createConsumer('service-account-kong-certificate-bound')
        ...
        await eventually(async () => {
            ...
            logResponse(resp)
            expect(resp.status, 'Status should be 200').to.equal(200);
            expect(resp.data.headers, 'Body.headers should have X-Consumer-Username').to.have.property('X-Consumer-Username');
            expect(resp.data.headers['X-Consumer-Username'], 'X-Consumer-Username should be true').to.equal('service-account-kong-certificate-bound');
        })

    })

    it('should fail authentication when consumer is removed and consumer_claim, default consumer_by is used', async function () {
        await deleteConsumer('service-account-kong-certificate-bound')
    ...
    it('should succeed when anonymous is set and the request does not include correct credentials', async function () {
        // update plugin to use anonymous consumer
        const payload = {
            config: {
        ...
        // request should fail as no valid anonymous consumer is created in kong
        await eventually(async () => {
            const resp = await axios({
                method: 'GET',
                url: `${urls.proxySec}${keycloakPath}`,
            ...
            expect(resp.data.message, 'Message should be "An unexpected error occurred"').to.equal('An unexpected error occurred')
        })
        ...
        await eventually(async () => {
            ...
            logResponse(resp)
            expect(resp.status, 'Status should be 200').to.equal(200);
            expect(resp.data.headers, 'Body.headers should have X-Anonymous-Consumer').to.have.property('X-Anonymous-Consumer');
            expect(resp.data.headers['X-Anonymous-Consumer'], 'X-Anonymous-Consumer should be true').to.equal('true');
            expect(resp.data.headers, 'Body.headers should have X-Consumer-Username').to.have.property('X-Consumer-Username');
            expect(resp.data.headers['X-Consumer-Username'], 'X-Consumer-Username should be true').to.equal('anonymous-tester');
            expect(resp.data.headers, 'Body.headers should have X-Consumer-Id').to.have.property('X-Consumer-Id');
        })
    })

    it('should delete tls plugin', async function () {
        ...
        const resp = await axios({
            ...
            data: {
                config: {
                    ...
                    consumer_claim: [], // reset consumer_claim to empty array
                    anonymous: null, // remove anonymous consumer
                },
            },
            validateStatus: null,
        })
    ...
    it('should successfully authenticate when accessing route with mTLS authentication', async function() {
        //Skip the test case because of https://konghq.atlassian.net/browse/KAG-7993
