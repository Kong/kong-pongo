...
const MAX_DURATION_PER_GROUP = Number(process.env.MAX_DURATION_PER_GROUP) || 900; // 15 minutes default
...
if (testsToSkip.size > 0) {
  console.error(`Total tests to skip on branch ${currentBranch} is ${Array.from(testsToSkip).join(', ')}`);
}
