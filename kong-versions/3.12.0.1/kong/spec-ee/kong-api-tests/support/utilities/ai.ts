    ...
    try {
      ...
      if (resp.status === 400) {
        ...
        if (msg.includes('The upstream server is timing out')) {
          ...
          throw new Error(`Received expected 504 timeout from ${provider}, skipping failure`);
        }
...
/**
 * Retries SDK client requests (e.g., AWS SDK) with exponential backoff
 * This function is designed for non-Axios clients like AWS SDK's BedrockAgentRuntimeClient
 * @param makeRequest Function that makes the request and returns a Promise
 * @param validateResponse Function that validates and returns the response
 * @param options Configuration options
 * @returns The validated response from the successful request
 */
export const retrySDKRequest = async <T>(
  makeRequest: () => Promise<T>,
  validateResponse: (resp: T) => any,
  options?: {
    provider?: string;
    timeout?: number;
    interval?: number;
  ...
  const shouldRetry = options?.shouldRetry ?? ((error: any) => {
    ...
    // Retry on network errors, timeouts, throttling
    return (
  ...
  while ((Date.now() - start) < timeout) {
    try {
      ...
      // Exponential backoff with jitter
      const delay = Math.min(interval * Math.pow(2, attempt), maxDelay) +
  ...
  throw lastError || new Error(`${provider} SDK request timed out after ${timeout}ms`);
};
...
export const createAILogCollectingRoute = async (
  ...
  // note the different ID/naming in the colleting plugin and exposing plugin
  // this is specically designed to allow either using a single AI proxy advanced plugin
  // or multiple plugins in the test
...
ngx.shared.kong:set("ai_last_serialized_log_${name}", encode_value(logs and logs.ai or {}))
  ...
  let resp = await axios({
    method: 'put',
    url: `${getUrl('routes')}/${routeIdWithAIPlugin}/plugins/${uuidv5(routeIdWithAIPlugin, uuidv5.DNS)}`,
    data: {
      name: 'post-function',
      config:{
        log: [luaCode],
  ...
  const routeResp = await axios({
    method: 'put',
    url: `${getUrl('routes')}/ai-collecting-route-${name}`,
    data: {
      paths: [collectRoutePath],
    },
    validateStatus: null
  })
  expect(routeResp.status, `Status should be 200`).equal(200);

  // the exposure service
  resp = await axios({
    method: 'put',
    url: `${getUrl('routes')}/${routeResp.data.id}/plugins/pre-function-${name}`,
    data: {
      name: 'pre-function',
      config:{
        access: [
...
export const deleteAILogCollectingRoute = async (
  name: string,
) => {
  return await deleteGatewayRoute(`ai-collecting-route-${name}`);
};
