...
You can set the `composeGeneratorFolderPath` environment variable to set the local path to the docker compose generator repo in your local env. This will be used in the `makefile`to start and stop containers in the specs.
...
- [AWS IAM Role Credentials](https://start.1password.com/open/i?a=KJVYOL2OTVGRPAAAHEVOL6MXZE&v=q7r4hh4465zentymwtoonxxp3m&i=4dchs3xjus2jqjvhkepyxwgk2e&h=team-kong.1password.com) - required only for `ai-aws-guardrails` test, credentials for ai-gateway-assumerole-user that used in AssumeRole tests

  `IAM_ROLE_AWS_ACCESS_KEY_ID`="<iam_role_aws_access_key_id>"`

  `IAM_ROLE_AWS_SECRET_ACCESS_KEY`="<iam_role_aws_secret_access_key>"`
...
- `ai-proxy` test
  ...
  [AZUREAI_REALTIME_API_KEY](https://start.1password.com/open/i?a=KJVYOL2OTVGRPAAAHEVOL6MXZE&v=q7r4hh4465zentymwtoonxxp3m&i=uyi7ym44hn36t7pcnq7hcmmrxm&h=team-kong.1password.com) `="<AZUREAI_REALTIME_API_KEY>"`

  [HUGGINGFACE_API_KEY](https://start.1password.com/open/i?a=KJVYOL2OTVGRPAAAHEVOL6MXZE&v=q7r4hh4465zentymwtoonxxp3m&i=px5srqeoq7zh7itsm7asl4szrq&h=team-kong.1password.com) `="<HUGGINGFACE_API_KEY>"`

  [COHERE_API_KEY](https://start.1password.com/open/i?a=KJVYOL2OTVGRPAAAHEVOL6MXZE&v=q7r4hh4465zentymwtoonxxp3m&i=vlas5hk722ccc7qscu7vt7dftu&h=team-kong.1password.com) `="<COHERE_API_KEY>"`
...
- `confluent` and `confluent-consume` tests
  ...
  [CONFLUENT_CLOUD_SR_PASSWORD](https://start.1password.com/open/i?a=KJVYOL2OTVGRPAAAHEVOL6MXZE&v=q7r4hh4465zentymwtoonxxp3m&i=troauac327rttd6idzw54lwa7q&h=team-kong.1password.com) `="<confluent-cloud-sr-password>"`
...
`export VERBOSE_CONFIG_REBUILD_LOGS=true` to enable logging inside waitForConfigRebuild func(default is `false`).
...
# CI workflow, select-test logic:

The CI system now creates test groups to execute the entire test suite or specific subgroups such as smoke, non-smoke, weekly, or all-except-ai. These groups are formed based on duration, with each group currently limited to a maximum of 15 minutes.

The test group generation logic utilizes a duration dictionary, located at `./gateway/shared/test_file_durations.json`, which maps test specs to their execution durations.

To refresh this JSON file with updated values or new specifications, download all hybrid mode test results XML files (as they typically have longer durations) and place them in your local results directory. Then, set the environment variable `READ_XML_TEST_RESULTS` to true and run this script `./support/script/test-results.sh` to generate a new, updated JSON file.
