...
if [ "$HTTP_STATUS" = "200" ] || [ "$HTTP_STATUS" = "204" ]; then
  ...
  # Without a server certificate the broker still accepts TCP on the TLS
  # ports but resets every handshake. Stop here, so the caller sees the real
  # cause instead of an SSL EOF in each tcps:// test.
  echo "ERROR: server certificate configuration returned HTTP: $HTTP_STATUS"
...
if [ "$HTTP_STATUS" = "200" ] || [ "$HTTP_STATUS" = "204" ]; then
  ...
  echo "ERROR: failed to enable TLS services. HTTP status: $HTTP_STATUS"
  # Get detailed error
