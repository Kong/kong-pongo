...
describe('Gateway Plugins: jwt-signer', function () {
  ...
  it('should proxy request with a valid token', async function () {
    ...
    expect(resp.status, 'Status should be 200').to.equal(200);
    expect(resp.data.headers[upstreamConsumerHeaderName], 'Should see consumer username in upstream request').to.equal(
      consumerName,
    );
  ...
  it('should patch jwt-signer plugin to disable auth and allow requests', async function () {
    ...
    expect(resp.data.config.channel_token_optional, 'Should be true').to.be.true;
