...
local VAULTS = {
...

  {
    ...
    setup = function(self)
      ...
      local opts = {
        auth_method_order = os.getenv("GCP_AUTH_METHOD_ORDER") or "legacy",
      }

      self.GCP = gcp()
      self.config.project_id = assert(cjson.decode(service_account).project_id)
      self.access_token = access_token:new(service_account, opts)
    end,
...
for i = 1, #VAULTS do
  setmetatable(VAULTS[i], harness_mt)
end

_M.get_vaults = function()
  return deepcopy(VAULTS)
end

_M.get_vault = function(name)
