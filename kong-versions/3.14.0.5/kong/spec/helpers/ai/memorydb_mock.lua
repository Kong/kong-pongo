          ...
          return {
            ...
            ["FT.SEARCH"] = function(red, index, ...)
              ...
              for _key, value in pairs(data) do
                ...
                table.insert(payloads, { "$", {}, "score", tostring(distance), key = _key })
                end
              end
              ...
              for i = 1, #payloads do
                local key = payloads[i].key
                payloads[i].key = nil -- remove the key from the payload
                table.insert(res, key) -- the first array is the keys
