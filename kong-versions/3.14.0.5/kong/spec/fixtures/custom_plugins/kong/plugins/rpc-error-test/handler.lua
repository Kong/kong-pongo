...
function RpcErrorTestHandler:init_worker()
  ...
  -- if rpc(cp) is unavailable a method should fail
  worker_events.register(function(capabilities_list)
    local res, err = kong.rpc:call(node_id, "kong.test.unavailable")
    assert(not res)
    assert(err == "no connection with control plane")

    ngx.log(ngx.DEBUG, "test #3 ok")
  end, "clustering:jsonrpc", "connection_failed")
