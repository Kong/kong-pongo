...
local function mock_embeddings(opts, path)
...
local function mock_embeddings_bedrock(opts, url)
  if opts.method ~= "POST" then
    return nil, "Only POST method is supported"
  end
  ...
  return { embedding = embedding }, { ["X-Amzn-Bedrock-Input-Token-Count"] = "8" }
...
local function mock_openai_embeddings(opts)
  return make_response({
    data = {
      { embedding = assert(mock_embeddings(opts, "input")) },
    },
    usage = {
      prompt_tokens = 8,
      total_tokens = 8
...
local function mock_huggingface_embeddings(opts)
  return make_response({ { assert(mock_embeddings(opts, "inputs")) } }, {
    ["x-compute-tokens"] = 8,
  })
end

local function mock_bedrock_embeddings(opts, url)
  return make_response(assert(mock_embeddings_bedrock(opts, url)))
end
