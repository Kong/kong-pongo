...
/**
 * Patch keys for JWE Decrypt plugin
 * @param {object} keysPayload - request body for patching keys
 * @returns {AxiosResponse}
 */
export const patchEncryptedKeysForJweDecryptPlugin = async (
  keyId: string,
  keysPayload: object
) => {
  const resp = await axios({
    method: 'patch',
    url: `${getUrl('keys')}/${keyId}`,
    data: keysPayload,
    validateStatus: null
  });
  logResponse(resp);
