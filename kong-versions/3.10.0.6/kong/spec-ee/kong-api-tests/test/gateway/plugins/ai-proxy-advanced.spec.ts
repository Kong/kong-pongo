    ...
    const assertions = (resp) => {
      ...
      if (resp.status === 400) {
        ...
        if (msg.includes('The upstream server is timing out')) {
          // Known/acceptable 504 error, do not fail or retry
        ...
        // Acceptable, do nothing (or add more checks if needed)
      } else {
        expect.fail(`Unexpected response status: ${resp.status}`);
      }
    };

    const resp = await retryRequest(req, assertions, 30000, 500);
