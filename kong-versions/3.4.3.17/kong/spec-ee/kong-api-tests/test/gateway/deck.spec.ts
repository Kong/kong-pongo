...
describe('decK: Sanity Tests', function () {
  ...
  it('should do a dump', async function () {
    const result = execCustomCommand(constructDeckCommand('dump --format json --yes "-o=kong"'));
    expect(result.stderr, 'deck ping error').to.be.undefined;
    ...
    expect(conf.services && conf.services.length > 0, 'deck dump does not contain any services').to.be.true;
    ...
    expect(service_matched, 'deck dump did not contain the expected service').equals(true);
    expect(route_matched, 'deck dump did not contain the expected route').equals(true);

    /* check if known consumer is found */

    expect(conf.consumers && conf.consumers.length > 0, 'deck dump does not contain any consumers').to.be.true;
    ...
    expect(consumer_matched, 'deck dump did not contain the expected consumer').equals(true);
    expect(cred_matched, 'deck dump did not contain the expected credential').equals(true);
  });
  ...
  it('should sync the db', async function () {
    const result = execCustomCommand(constructDeckCommand(`sync ./${deckFileName}`));
    expect(result.stderr, 'deck ping error').to.be.undefined;
  ...
  it('should detect drift', async function () {
    let result = execCustomCommand(constructDeckCommand(`diff ./${deckFileName} --non-zero-exit-code`));
    ...
    result = execCustomCommand(constructDeckCommand(`diff ./${deckFileName} --non-zero-exit-code`));

    expect(result.stderr, 'deck diff failed').to.be.not.undefined;
    expect(result.status, 'deck diff did not terminate with right exit code').equals(2);
  });
