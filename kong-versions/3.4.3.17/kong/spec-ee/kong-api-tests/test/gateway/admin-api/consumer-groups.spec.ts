...
describe('@gke: Gateway Consumer Groups with RLA', function () {
  ...
  it('should not create a consumer group with empty name', async function () {
    ...
    expect(resp.data.fields.name, 'Should have correct error field name').to.equal('length must be at least 1');
  });
  ...
  it('should not create a consumer group without name', async function () {
    ...
    expect(resp.data.fields.name, 'Should have correct error field name').to.equal('required field missing');
  });
  ...
  it('should see the consumer group 1 in groups list', async function () {
    ...
    expect(resp.data, 'Should not have empty consumer group list').not.to.be.ofSize(0);

    for (const group of resp.data.data) {
      if (group.id === consumerGroup1.id) {
        expect(group.name, 'Should see the 1st group name in the list').to.equal(consumerGroup1.name);
        found = true;
      }
    }

    if (!found) {
      throw new Error('The consumer group 1 was not found in consumer groups list');
    }
  });
  ...
  it('should not add non-existing consumer to a consumer group', async function () {
    ...
    expect(resp.data.message, 'Should have correct error message').to.eq(`Consumer 'nonExisting' not found`);
  });
  ...
  it('should not add consumer to a non-existing consumer group', async function () {
    ...
    expect(resp.status, 'Status should be 404').to.equal(404);
    expect(resp.data.message, 'Should have correct error message').to.eq(`Group 'wrongGroup' not found`);
  });
  ...
  it('should not add consumer to a consumer group with empty body', async function () {
    ...
    expect(resp.data.message, 'Should have correct error message').to.eq('must provide consumer');
  });
  ...
  it('should add consumer 1 to consumer group 1 using consumer_groups endpoint', async function () {
    ...
    const assertions = resp => {
      ...
      expect(resp.status, 'Status should be 201').to.equal(201);
      expect(resp.data.consumers[0].username, 'Should have correct username').to.eq(consumer1.username);
      expect(resp.data.consumers[0].username_lower, 'Should have correct username_lower').to.eq(
        consumer1.username_lower,
      );
      expect(resp.data.consumers[0].id, 'Should have correct consumer id').to.eq(consumer1.id);
      expect(resp.data.consumers[0].created_at, 'Should have created_at').to.be.a('number');
    };
  ...
  it('should not add consumer 1 to consumer group 1 2nd time', async function () {
    const req = () =>
      postNegative(`${url}/${consumerGroup1.name}/consumers`, {
        consumer: consumer1.username,
      });
  ...
  it('should define settings for consumer group 1', async function () {
    ...
    const assertions = resp => {
      expect(resp.status, 'Status should be 201').to.equal(201);
      expect(resp.data.plugin, 'Should have correct plugin name').to.eq('rate-limiting-advanced');
      expect(resp.data.consumer_group, 'Should have correct consumer group name').to.eq(consumerGroup1.name);
      expect(resp.data.config.limit, 'Should have correct limit').to.be.equalTo([2]);
      expect(resp.data.config.window_size, 'Should have correct window_size').to.be.equalTo([5]);
    };
  ...
  it('should not define settings for a group without limit', async function () {
    ...
    expect(resp.data.fields.config.window_size, 'Should have window_size in error').to.equal('required field missing');
  });
  ...
  it('should not define settings for a group without window_size', async function () {
    ...
    expect(resp.data.fields.config.limit, 'Should have limit in error').to.equal('required field missing');
  });
  ...
  it('should not define settings for a group with unequal window_size and limit', async function () {
    ...
    const req = () =>
      postNegative(`${url}/${consumerGroup1.name}/overrides/plugins/rate-limiting-advanced`, data, 'put');
    ...
    const assertions = resp => {
      ...
      expect(resp.data.fields['@entity'][0], 'Should have limit in error').to.equal(
        'You must provide the same number of windows and limits',
      );
    };
  ...
  it('should not define settings for a group with non-array limit', async function () {
    ...
    const assertions = resp => {
      expect(resp.status, 'Status should be 400').to.equal(400);
      expect(resp.data.message, 'Should have correct error message').to.contain('config.limit: expected an array');
      expect(resp.data.fields.config.limit, 'Should have limit error').to.equal('expected an array');
    };
  ...
  it('should not define settings for a group with non-array window_size', async function () {
    ...
    expect(resp.status, 'Status should be 400').to.equal(400);
    expect(resp.data.message, 'Should have correct error message').to.contain('config.window_size: expected an array');
    expect(resp.data.fields.config.window_size, 'Should have window_size error').to.equal('expected an array');
  });
  ...
  it('should get consumer group 1 specific details', async function () {
    ...
    const assertions = resp => {
      assertConsumergroupResponse(resp.data.consumer_group, consumerGroup1.name);
      expect(resp.data.consumers, 'Should not have empty consumer group list').not.to.be.ofSize(0);
    };
    ...
    for (const consumer of resp.data.consumers) {
      if (consumer.id === consumer1.id) {
        expect(consumer.username, 'Should see the 1st consumer name in the group details').to.equal(consumer1.username);
        expect(consumer.username_lower, 'Should have correct username_lower').to.equal(consumer1.username_lower);
        isConsumer = true;
      }
    }
    ...
    expect(resp.data.plugins, 'Should not have empty consumer group list').not.to.be.ofSize(0);

    for (const plugin of resp.data.plugins) {
      if (plugin.consumer_group.id === consumerGroup1.id) {
        expect(plugin.config.limit, 'Should have correct limit for the consumer group').to.be.equalTo([2]);
        expect(plugin.config.window_size, 'Should have correct window_size for the consumer group').to.be.equalTo([5]);
        expect(plugin.config.retry_after_jitter_max, 'Should have default jitter number').to.eq(0);
        expect(plugin.config.window_type, 'Should have deafult sliding window_type').eq('sliding');
        expect(plugin.name, 'Should have correct plugin name').eq('rate-limiting-advanced');
        expect(plugin.id, 'Should have plugin id').to.be.a('string');
        expect(plugin.created_at, 'Should have created_at number').to.be.a('number');
        isPlugin = true;
      }
    }

    if (!isPlugin) {
      throw new Error('The plugin was not found in plugins list of the consumer group 1');
    }
  });
  ...
  it('should not get non-existing consumer group specific details', async function () {
    ...
    expect(resp.status, 'Status should be 404').to.equal(404);
    expect(resp.data.message, 'Should have correct error message').to.equal(`Group 'wrong' not found`);
  });
  ...
  it('should define settings for consumer group 1 with jitter_max', async function () {
    ...
    const assertions = resp => {
      ...
      expect(resp.data.plugin, 'Should have correct plugin name').to.eq('rate-limiting-advanced');
      expect(resp.data.consumer_group, 'Should have correct consumer group name').to.eq(consumerGroup1.name);
      expect(resp.data.config.limit, 'Should have correct limit').to.be.equalTo([25, 15]);
      expect(resp.data.config.window_size, 'Should have correct window_size').to.be.equalTo([55, 25]);
      expect(resp.data.config.retry_after_jitter_max, 'Should have correct jitter_max').to.eq(10);
    };
  ...
  it('should add consumer 2 to consumer group 2 using consumers endpoint', async function () {
    const consumerUrl = url.replace('/consumer_groups', `/consumers/${consumer2.username}/consumer_groups`);
    ...
    expect(resp.data.consumer_groups, 'Should have 1 consumer group in the list').to.be.ofSize(1);

    assertConsumergroupResponse(resp.data.consumer_groups[0], consumerGroup2.name);

    expect(resp.status, 'Status should be 201').to.equal(201);
    expect(resp.data.consumer.username, 'Should have correct username').to.eq(consumer2.username);
    expect(resp.data.consumer.username_lower, 'Should have correct username_lower').to.eq(consumer2.username_lower);
    expect(resp.data.consumer.id, 'Should have correct consumer id').to.eq(consumer2.id);
    expect(resp.data.consumer.created_at, 'Should have created_at').to.be.a('number');
    expect(resp.data.consumer.custom_id, 'Should have custom_id null').to.be.null;
  });

  it('should get all consumer groups of the consumer 1', async function () {
    const consumerUrl = url.replace('/consumer_groups', `/consumers/${consumer1.username}/consumer_groups`);
    const resp = await axios(consumerUrl);
    logResponse(resp);

    expect(resp.status, 'Status should be 200').to.equal(200);
    expect(resp.data.consumer_groups, 'Should belong to only 1 consumer_group').to.be.ofSize(1);
    expect(resp.data.consumer_groups[0].name, 'Should have consumer_group 1 name').to.equal(consumerGroup1.name);
    expect(resp.data.consumer_groups[0].id, 'Should have consumer_group 1 id').to.equal(consumerGroup1.id);
  });
  ...
  it('should add consumer 1 to consumer group 2 using consumers endpoint', async function () {
    ...
    expect(resp.data.consumer_groups, 'Should see 1 consumer group in the list').to.be.ofSize(1);

    assertConsumergroupResponse(resp.data.consumer_groups[0], consumerGroup2.name);

    expect(resp.status, 'Status should be 201').to.equal(201);
    expect(resp.data.consumer.username, 'Should have correct username').to.eq(consumer1.username);
    expect(resp.data.consumer.username_lower, 'Should have correct username_lower').to.eq(consumer1.username_lower);
    expect(resp.data.consumer.id, 'Should have correct consumer id').to.eq(consumer1.id);
    expect(resp.data.consumer.created_at, 'Should have created_at').to.be.a('number');
    expect(resp.data.consumer.custom_id, 'Should have custom_id null').to.be.null;
  });
  ...
  it('should not get all consumer groups of a wrong consumer', async function () {
    const consumerUrl = url.replace('/consumer_groups', `/consumers/wrong/consumer_groups`);
    const resp = await postNegative(consumerUrl);
    logResponse(resp);

    expect(resp.status, 'Status should be 404').to.equal(404);
    expect(resp.data.message, 'Should have correct error message').to.equal(`Consumer 'wrong' not found`);
  });
  ...
  it('should see consumer 2 in consumers list of consumer group 2', async function () {
    ...
    expect(resp.status, 'Status should be 200').to.equal(200);
    expect(resp.data.consumers, 'Should have 1 consumer in consumers list').to.be.ofSize(1);
    expect(resp.data.consumers[0].username, 'Should have correct username').to.equal(consumer2.username);
    expect(resp.data.consumers[0].username_lower, 'Should have correct username_lower').to.equal(
      consumer2.username_lower,
    );
    expect(resp.data.consumers[0].id, 'Should have correct id').to.equal(consumer2.id);
  });
  ...
  it('should not delete wrong consumer from a target consumer group', async function () {
    const consumerUrl = url.replace('/consumer_groups', `/consumers/wrong/consumer_groups/${consumerGroup2.id}`);
  ...
  it('should add multiple consumers to consumer group 2', async function () {
    ...
    expect(resp.status, 'Status should be 201').to.equal(201);
    expect(resp.data.consumers, 'Should have 2 consumers in the group').to.be.ofSize(2);
    expect(resp.data.consumer_group.name, 'Should have name of consumer group 2').to.eq(consumerGroup2Name);
    ...
    for (const consumer of resp.data.consumers) {
      if (consumer.username === consumer1.username) {
        ...
        throw new Error(`Consumer username for ${JSON.stringify(consumer)} was not found in the response`);
      }
    }
  });
  ...
  it('should not delete non existing consumer from consumer group 2 using groups endpoint', async function () {
    const resp = await postNegative(`${url}/${consumerGroup2.name}/consumers/wrong`, {}, 'delete');
    logResponse(resp);
  ...
  it('should add the same consumer 1 to 2 groups', async function () {
    ...
    const assertions = resp => {
      expect(resp.status, 'Status should be 201').to.equal(201);
      expect(resp.data.consumer_groups, 'Should see 1 consumer group in the list').to.be.ofSize(2);
      ...
      for (const consumerGroup of resp.data.consumer_groups) {
        if (consumerGroup.id === consumerGroup1.id) {
          ...
          throw new Error(`Consumer group name for ${JSON.stringify(consumerGroup)} was not found in the response`);
        }
      }
  ...
  it('should not add the consumer 1 to non-existing group', async function () {
    ...
    const req = () =>
      postNegative(consumerUrl, {
        group: ['non-existing-group'],
      });
    const assertions = resp => {
      expect(resp.status, 'Status should be 404').to.equal(404);
      expect(resp.data.message, 'Should have correct error message').to.equal("Group 'non-existing-group' not found");
    };
  ...
  it('should not delete a consumer from non existing consumer group using groups endpoint', async function () {
    const resp = await postNegative(`${url}/wrong/consumers/${consumer1.username}`, {}, 'delete');
    logResponse(resp);
